...
/**
 * WebAppCommand creates an Mod Web application at the specified location.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class WebAppCommand extends CConsoleCommand
{
	...
	public function getHelp()
	{
		return <<<EOD
USAGE
  modc webapp <app-path> [<vcs>]

DESCRIPTION
  This command generates an Mod Web Application at the specified location.
...
EOD;
	...
	protected function setPermissions($targetDir)
	{
		...
		@chmod($targetDir.'/protected/modc',0755);
	}
	...
	protected function addFileModificationCallbacks(&$fileList)
	{
		...
		$fileList['protected/modc.php']['callback']=array($this,'generateModc');
	}

	/**
	 * Inserts path to framework's mod.php into application's index.php
	 *
	...
	public function generateIndex($source,$params)
	{
		...
		$mod=realpath(dirname(__FILE__).'/../../Mod.php');
		$mod=$this->getRelativePath($mod,$this->_rootPath.DIRECTORY_SEPARATOR.'index.php');
		$mod=str_replace('\\','\\\\',$mod);
		return preg_replace('/\$mod\s*=(.*?);/',"\$mod=$mod;",$content);
	}

	/**
	 * Inserts path to framework's modt.php into application's index-test.php
	 *
	 * @param string $source source file path
	 * @param array $params
	 * @return string modified source file content
	...
	{
		$content=file_get_contents($source);
		$mod=realpath(dirname(__FILE__).'/../../modt.php');
		$mod=$this->getRelativePath($mod,$this->_rootPath.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'tests'.DIRECTORY_SEPARATOR.'bootstrap.php');
		$mod=str_replace('\\','\\\\',$mod);
		return preg_replace('/\$modt\s*=(.*?);/',"\$modt=$mod;",$content);
	}

	/**
	 * Inserts path to framework's modc.php into application's modc.php
	 *
	 * @param string $source source file path
	 * @param array $params
	 * @return string modified source file content
	 */
	public function generateModc($source,$params)
	{
		$content=file_get_contents($source);
		$modc=realpath(dirname(__FILE__).'/../../modc.php');
		$modc=$this->getRelativePath($modc,$this->_rootPath.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'modc.php');
		$modc=str_replace('\\','\\\\',$modc);
		return preg_replace('/\$modc\s*=(.*?);/',"\$modc=$modc;",$content);
	}
