	...
	public function run($args)
	{
		...
		if(empty($matches[1]))  // without regular expression
		{
			...
			if(($pos=strrpos($className,'.'))===false)
				$basePath=Mod::getPathOfAlias('application.models');
			else
			{
				$basePath=Mod::getPathOfAlias(substr($className,0,$pos));
		...
		else  // with regular expression
		{
			...
			if($pos>0)  // only regexp is given
				$basePath=Mod::getPathOfAlias(rtrim(substr($className,0,$pos),'.'));
			else
				$basePath=Mod::getPathOfAlias('application.models');
		...
		$templatePath=$this->templatePath===null?MOD_PATH.'/cli/views/shell/model':$this->templatePath;
		$fixturePath=$this->fixturePath===null?Mod::getPathOfAlias('application.tests.fixtures'):$this->fixturePath;
		$unitTestPath=$this->unitTestPath===null?Mod::getPathOfAlias('application.tests.unit'):$this->unitTestPath;
...
EOD;
	...
	public function generateModel($source,$params)
	{
		...
		if(!is_file($source))  // fall back to default ones
			$source=MOD_PATH.'/cli/views/shell/model/'.basename($source);
	...
	{
		$tablePrefix=Mod::app()->getDb()->tablePrefix;
