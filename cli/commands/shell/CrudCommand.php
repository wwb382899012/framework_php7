...
EOD;
	...
	public function run($args)
	{
		...
		else
		{
			...
			if(($m=Mod::app()->getModule($id))!==null)
				$module=$m;
		}
		$modelClass=Mod::import($modelClass);
		...
		if(isset($args[1]))
		{
			...
			else
			{
				...
				if(($m=Mod::app()->getModule($first))!==null)
					$module=$m;
				else
				{
		...
		$templatePath=$this->templatePath===null?MOD_PATH.'/cli/views/shell/crud':$this->templatePath;
		$functionalTestPath=$this->functionalTestPath===null?Mod::getPathOfAlias('application.tests.functional'):$this->functionalTestPath;
	...
	public function generateController($source,$params)
	{
		...
		if($id===null)
			throw new ShellException(Mod::t('mod','Error: Table "{table}" does not have a primary key.',array('{table}'=>$model->tableName())));
		elseif(is_array($id))
			throw new ShellException(Mod::t('mod','Error: Table "{table}" has a composite primary key which is not supported by crud command.',array('{table}'=>$model->tableName())));

		if(!is_file($source))  // fall back to default ones
			$source=MOD_PATH.'/cli/views/shell/crud/'.basename($source);
