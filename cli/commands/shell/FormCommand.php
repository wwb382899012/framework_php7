...
EOD;
	...
	public function run($args)
	{
		...
		$modelClass=Mod::import($args[0],true);
		$model=new $modelClass($scenario);
		$attributes=$model->getSafeAttributeNames();

		$templatePath=$this->templatePath===null?MOD_PATH.'/cli/views/shell/form':$this->templatePath;
		$viewPath=Mod::getPathOfAlias($args[1]);
		...
		if(!is_file($actionFile))  // fall back to default ones
			$actionFile=MOD_PATH.'/cli/views/shell/form/action.php';
	...
	public function generateForm($source,$params)
	{
		if(!is_file($source))  // fall back to default ones
			$source=MOD_PATH.'/cli/views/shell/form/'.basename($source);
