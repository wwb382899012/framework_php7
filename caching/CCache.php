...
abstract class CCache extends CApplicationComponent implements ICache, ArrayAccess
{
	 ...
	 * key will not be tampered when calling the relevant methods {@link get()}, {@link set()}, {@link add()} and {@link delete()}. This is useful if a Mod
	...
	public function init()
	{
		...
		if($this->keyPrefix===null)
            $this->keyPrefix=isset(Mod::app()->params['cacheKeyPrefix'])?Mod::app()->params['cacheKeyPrefix']:'';
			//$this->keyPrefix=Mod::app()->getId();
	}
	...
	public function get($id, $raw=false)
	{
		...
		if($this->serializer===null)
            ...
            $data=call_user_func($this->serializer[1], $value);
	...
	{
		Mod::trace('Saving "'.$id.'" to cache','system.caching.'.get_class($this));
	...
	{
		Mod::trace('Adding "'.$id.'" to cache','system.caching.'.get_class($this));
	...
	{
		Mod::trace('Deleting "'.$id.'" from cache','system.caching.'.get_class($this));
	...
	{
		Mod::trace('Flushing cache','system.caching.'.get_class($this));
	...
	{
		throw new CException(Mod::t('mod','{className} does not support get() functionality.',
	...
	{
		throw new CException(Mod::t('mod','{className} does not support set() functionality.',
			array('{className}'=>get_class($this))));
	}
	...
	{
		throw new CException(Mod::t('mod','{className} does not support add() functionality.',
			array('{className}'=>get_class($this))));
	}
	...
	{
		throw new CException(Mod::t('mod','{className} does not support delete() functionality.',
			array('{className}'=>get_class($this))));
	}
	...
	{
		throw new CException(Mod::t('mod','{className} does not support flushValues() functionality.',
			array('{className}'=>get_class($this))));
	}
