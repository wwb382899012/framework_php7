 ...
 *
 * @method exists($keyName)
 * @method hExists($keyName, $fieldName)
 * @method setex($keyName, $expire, $value)
 */
class CRedisCache extends CCache
{
	...
	protected $getFunctions = array(
		//key
		'dump', 'exists', 'keys', 'getKeys', 'object', 'pttl', 'randomKey', 'sort', 'ttl', 'type','watch', 
		//string
		...
		//set
		'sCard', 'sSize', 'sDiff', 'sInter', 'sIsMember', 'sContains', 'sMembers', 'sGetMembers', 'sRandMember', 'sUnion', 'sScan',
		//sorted set
    ...
    {
        //Mod::log($name.json_encode($parameters),"error");
        /*$idc = $this->_localIDC;
        ...
        if (in_array($name, $this->getFunctions))
            $res = $this->callRedisFunction(true, $name, $parameters);
        else if (in_array($name, $this->setFunctions))
            $res = $this->callRedisFunction(false, $name, $parameters);
        else
            $res = parent::__call();    //如果调用的不是Redis的方法，则交给父类处理
     ...
     * @param string $functionName 调用的方法名称
     * @param array $functionParams 参数列表
     * @return mixed
     * @throws CException
     */
	protected function callRedisFunction($isGet, $functionName, $functionParams)
	{
	    //Mod::log($functionName.json_encode($functionParams),"error");
		//获取需要操作的IDC列表
			...
			for($retryCount=0; $retryCount<=$retryCountSum; ++$retryCount)
			{
				try
				{
					...
					else
					{
						...
						{
							throw new CException($server->getLastError(), 500);
						}
						else
						{
							$writeSuccess = true;
	...
	public function getServer($IDCName)
	{
		...
		$pwd = $this->serverConfigs[$IDCName]['password'];
		$server = new Redis;
     ...
     * @param $pexpire int 过期时间，miliseconds
     * @return mixed
     * @throws CException
     */
	public function setNxPx($key,$value,$pexpire)
	{
		return $this->__call('set', array($key, $value, array('nx','px'=>$pexpire)));			
	}
