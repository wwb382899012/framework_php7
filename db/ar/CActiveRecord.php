 ...
 * Please check {@link http://www.modframework.com/doc/guide/database.ar the Guide} for more details
 * about this class.
 *
...
abstract class CActiveRecord extends CModel
{
	...
	public function getRelated($name,$refresh=false,$params=array())
	{
		...
		if(!isset($md->relations[$name]))
			throw new CDbException(Mod::t('mod','{class} does not have relation "{name}".',
				array('{class}'=>get_class($this), '{name}'=>$name)));

		Mod::trace('lazy loading '.get_class($this).'.'.$name,'system.db.ar.CActiveRecord');
	...
	public function getDbConnection()
	{
		...
		{
			self::$db=Mod::app()->getDb();
			if(self::$db instanceof CDbConnection)
				return self::$db;
			else
				throw new CDbException(Mod::t('mod','Active Record requires a "db" CDbConnection application component.'));
		}
	}
	...
	public function insert($attributes=null)
	{
		if(!$this->getIsNewRecord())
			throw new CDbException(Mod::t('mod','The active record cannot be inserted to database because it is not new.'));
		if($this->beforeSave())
		{
			Mod::trace(get_class($this).'.insert()','system.db.ar.CActiveRecord');
	...
	{
		if($this->getIsNewRecord())
			throw new CDbException(Mod::t('mod','The active record cannot be updated because it is new.'));
		if($this->beforeSave())
		{
			Mod::trace(get_class($this).'.update()','system.db.ar.CActiveRecord');
			if($this->_pk===null)
				$this->_pk=$this->getPrimaryKey();
	...
	public function saveAttributes($attributes)
	{
		if(!$this->getIsNewRecord())
		{
			Mod::trace(get_class($this).'.saveAttributes()','system.db.ar.CActiveRecord');
			$values=array();
	...
	{
		Mod::trace(get_class($this).'.saveCounters()','system.db.ar.CActiveRecord');
		$builder=$this->getCommandBuilder();
		$table=$this->getTableSchema();
	...
	public function delete()
	{
		if(!$this->getIsNewRecord())
		{
			Mod::trace(get_class($this).'.delete()','system.db.ar.CActiveRecord');
			...
			throw new CDbException(Mod::t('mod','The active record cannot be deleted because it is new.'));
	}
	...
	{
		Mod::trace(get_class($this).'.refresh()','system.db.ar.CActiveRecord');
	...
	{
		Mod::trace(get_class($this).'.find()','system.db.ar.CActiveRecord');
	...
	{
		Mod::trace(get_class($this).'.findAll()','system.db.ar.CActiveRecord');
		$criteria=$this->getCommandBuilder()->createCriteria($condition,$params);
	...
	{
		Mod::trace(get_class($this).'.findByPk()','system.db.ar.CActiveRecord');
	...
	{
		Mod::trace(get_class($this).'.findAllByPk()','system.db.ar.CActiveRecord');
		$prefix=$this->getTableAlias(true).'.';
		$criteria=$this->getCommandBuilder()->createPkCriteria($this->getTableSchema(),$pk,$condition,$params,$prefix);
		return $this->query($criteria,true);
	}
	...
	{
		Mod::trace(get_class($this).'.findByAttributes()','system.db.ar.CActiveRecord');
		$prefix=$this->getTableAlias(true).'.';
	...
	{
		Mod::trace(get_class($this).'.findAllByAttributes()','system.db.ar.CActiveRecord');
		$prefix=$this->getTableAlias(true).'.';
		$criteria=$this->getCommandBuilder()->createColumnCriteria($this->getTableSchema(),$attributes,$condition,$params,$prefix);
		return $this->query($criteria,true);
	}
	...
	{
		Mod::trace(get_class($this).'.findBySql()','system.db.ar.CActiveRecord');
		$this->beforeFind();
	...
	{
		Mod::trace(get_class($this).'.findAllBySql()','system.db.ar.CActiveRecord');
		$this->beforeFind();
		if(($criteria=$this->getDbCriteria(false))!==null && !empty($criteria->with))
		{
			$this->resetScope(false);
	...
	{
		Mod::trace(get_class($this).'.count()','system.db.ar.CActiveRecord');
		$this->beforeCount();
		$builder=$this->getCommandBuilder();
	...
	{
		Mod::trace(get_class($this).'.countByAttributes()','system.db.ar.CActiveRecord');
		$prefix=$this->getTableAlias(true).'.';
		$builder=$this->getCommandBuilder();
		$this->beforeCount();
	...
	{
		Mod::trace(get_class($this).'.countBySql()','system.db.ar.CActiveRecord');
		$this->beforeCount();
	...
	{
		Mod::trace(get_class($this).'.exists()','system.db.ar.CActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$table=$this->getTableSchema();
		$criteria->select='1';
	...
	{
		Mod::trace(get_class($this).'.updateByPk()','system.db.ar.CActiveRecord');
		$builder=$this->getCommandBuilder();
		$table=$this->getTableSchema();
	...
	{
		Mod::trace(get_class($this).'.updateAll()','system.db.ar.CActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
	...
	{
		Mod::trace(get_class($this).'.updateCounters()','system.db.ar.CActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$command=$builder->createUpdateCounterCommand($this->getTableSchema(),$counters,$criteria);
		return $command->execute();
	...
	{
		Mod::trace(get_class($this).'.deleteByPk()','system.db.ar.CActiveRecord');
		$builder=$this->getCommandBuilder();
	...
	{
		Mod::trace(get_class($this).'.deleteAll()','system.db.ar.CActiveRecord');
		$builder=$this->getCommandBuilder();
		$criteria=$builder->createCriteria($condition,$params);
		$command=$builder->createDeleteCommand($this->getTableSchema(),$criteria);
		return $command->execute();
	...
	{
		Mod::trace(get_class($this).'.deleteAllByAttributes()','system.db.ar.CActiveRecord');
		$builder=$this->getCommandBuilder();
		$table=$this->getTableSchema();
...
class CManyManyRelation extends CHasManyRelation
{
	...
	private function initJunctionData()
	{
		if(!preg_match('/^\s*(.*?)\((.*)\)\s*$/',$this->foreignKey,$matches))
			throw new CDbException(Mod::t('mod','The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key. The format of the foreign key must be "joinTable(fk1,fk2,...)".',
...
class CActiveRecordMetaData
{
	...
	public function __construct($model)
	{
		...
		if(($table=$model->getDbConnection()->getSchema()->getTable($tableName))===null)
			throw new CDbException(Mod::t('mod','The table "{table}" for active record class "{class}" cannot be found in the database.',
	...
	public function addRelation($name,$config)
	{
		if(isset($config[0],$config[1],$config[2]))  // relation class, AR class, FK
			...
			throw new CDbException(Mod::t('mod','Active record "{class}" has an invalid configuration for relation "{relation}". It must specify the relation type, the related active record class and the foreign key.', array('{class}'=>$this->_modelClassName,'{relation}'=>$name)));
	}
