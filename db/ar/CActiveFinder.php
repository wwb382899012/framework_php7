...
class CActiveFinder extends CComponent
{
	...
	{
		Mod::trace(get_class($this->_joinTree->model).'.findBySql() eagerly','system.db.ar.CActiveRecord');
	...
	{
		Mod::trace(get_class($this->_joinTree->model).'.findAllBySql() eagerly','system.db.ar.CActiveRecord');
	...
	{
		Mod::trace(get_class($this->_joinTree->model).'.count() eagerly','system.db.ar.CActiveRecord');
		$this->joinAll=$criteria->together!==true;
	...
	private function buildJoinTree($parent,$with,$options=null)
	{
		if($parent instanceof CStatElement)
			throw new CDbException(Mod::t('mod','The STAT relation "{name}" cannot have child relations.',
		...
		if(is_string($with))
		{
			...
			if(($relation=$parent->model->getActiveRelation($with))===null)
				throw new CDbException(Mod::t('mod','Relation "{name}" is not defined in active record class "{class}".',
	...
	private function applyLazyCondition($query,$record)
	{
		...
		if($this->relation instanceof CManyManyRelation)
		{
			...
			if(($joinTable=$schema->getTable($joinTableName))===null)
				throw new CDbException(Mod::t('mod','The relation "{relation}" in active record class "{class}" is not specified correctly: the join table "{joinTable}" given in the foreign key cannot be found in the database.',
			...
			if($parentCondition!==array() && $childCondition!==array())
			{
				...
				throw new CDbException(Mod::t('mod','The relation "{relation}" in active record class "{class}" is specified with an incomplete foreign key. The foreign key must consist of columns referencing both joining tables.',
		...
		else
		{
			...
			foreach($select as $name)
			{
				...
				elseif(preg_match('/^(.*?)\s+AS\s+(\w+)$/im',$name,$matches)) // if the column is already aliased
				{
					...
					throw new CDbException(Mod::t('mod','Active record "{class}" is trying to select an invalid column "{column}". Note, the column must exist in the table or be an expression with alias.',
	...
	private function joinOneMany($fke,$fks,$pke,$parent)
	{
		...
		foreach($fks as $i=>$fk)
		{
			...
			if(!isset($fke->_table->columns[$fk]))
				throw new CDbException(Mod::t('mod','The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key "{key}". There is no such column in the table "{table}".',
...
class CStatElement
{
	...
	private function queryOneMany()
	{
		...
		if(count($fks)!==$pkCount)
			throw new CDbException(Mod::t('mod','The relation "{relation}" in active record class "{class}" is specified with an invalid foreign key. The columns in the key must match the primary keys of the table "{table}".',
		...
		foreach($fks as $i=>$fk)
		{
			...
			if(isset($table->foreignKeys[$fk]))
			{
				...
				if($schema->compareTableNames($pkTable->rawName,$tableName))
					...
					throw new CDbException(Mod::t('mod','The relation "{relation}" in active record class "{class}" is specified with a foreign key "{key}" that does not point to the parent table "{table}".',
