...
class CDbCommandBuilder extends CComponent
{
	...
	{
		if (empty($data))
			throw new CDbException(Mod::t('mod','Can not generate multiple insert command with empty data set.'));
	...
	public function createUpdateCommand($table,$data,$criteria)
	{
		...
		if($fields===array())
			throw new CDbException(Mod::t('mod','No columns are being updated for table "{table}".',
	...
	public function createUpdateCounterCommand($table,$counters,$criteria)
	{
		...
		if($fields!==array())
		{
			...
			throw new CDbException(Mod::t('mod','No counter columns are being updated for table "{table}".',
				array('{table}'=>$table->name)));
	}
	...
	public function createColumnCriteria($table,$columns,$condition='',$params=array(),$prefix=null)
	{
		...
		foreach($columns as $name=>$value)
		{
			if(($column=$table->getColumn($name))!==null)
			{
				...
				throw new CDbException(Mod::t('mod','Table "{table}" does not have a column named "{column}".',
	...
	public function createInCondition($table,$columnName,$values,$prefix=null)
	{
		...
		elseif(is_array($columnName)) // composite key: $values=array(array('pk1'=>'v1','pk2'=>'v2'),array(...))
		{
			foreach($columnName as $name)
			{
				...
				for($i=0;$i<$n;++$i)
				{
					if(isset($values[$i][$name]))
					{
						...
						throw new CDbException(Mod::t('mod','The value for the column "{column}" is not supplied when querying the table "{table}".',
							array('{table}'=>$table->name,'{column}'=>$name)));
				}
			}
			if(count($values)===1)
			...
			throw new CDbException(Mod::t('mod','Column name must be either a string or an array.'));
	}
	...
	protected function ensureTable(&$table)
	{
		if(is_string($table) && ($table=$this->_schema->getTable($tableName=$table))===null)
			throw new CDbException(Mod::t('mod','Table "{table}" does not exist.',
