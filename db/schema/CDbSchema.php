...
abstract class CDbSchema extends CComponent
{
	...
	public function getTable($name,$refresh=false)
	{
		...
		else
		{
			...
			if(!isset($this->_cacheExclude[$name]) && ($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=Mod::app()->getComponent($this->_connection->schemaCacheID))!==null)
			{
				$key='mod:dbschema'.$this->_connection->connectionString.':'.$this->_connection->username.':'.$name;
				$table=$cache->get($key);
	...
	{
		if(($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=Mod::app()->getComponent($this->_connection->schemaCacheID))!==null)
		{
	...
	{
		throw new CDbException(Mod::t('mod','{class} does not support fetching all table names.',
