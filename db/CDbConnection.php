...
class CDbConnection extends CApplicationComponent
{
	...
	public function init()
	{
        ...
        //合并数据库配置
        $this->allConfigs['master'] = array('connectionString'=>$this->connectionString, 'username'=>$this->username, 'password'=>$this->password );
        $this->_pdos['master'] = null;
        foreach($this->slaveConfigs as $key=>$config)
        {
            $dbId = 'slave'.($key+1);
            $this->allConfigs[$dbId] = array('connectionString'=>$config['connectionString'], 'username'=>$config['username'], 'password'=>$config['password']);
     ...
     * @throws CException if connection fails
     */
	protected function open($dbId='master')
	{
	    if(empty($dbId))
            throw new CDbException('dbId cannot be empty.');

		if($this->_pdos[$dbId]===null)
		{
            if(empty($this->allConfigs[$dbId]['connectionString']))
				throw new CDbException('CDbConnection.'.$dbId.'.connectionString cannot be empty.');
			try
			{
				Mod::trace('Opening DB connection','system.db.CDbConnection');
                $this->_pdos[$dbId]=$this->createPdoInstance($dbId);
                $this->_pdo=$this->_pdos[$dbId];
	 ...
	 */
	protected function createPdoInstance($dbId='master')
	{
        if($this->_pdos[$dbId]===null)
        {
            ...
            if (!class_exists($pdoClass))
                throw new CDbException(Mod::t('mod', 'CDbConnection is unable to find PDO class "{className}". Make sure PDO is installed correctly.',
                                              array('{className}' => $pdoClass)));

            @$instance = new $pdoClass($this->allConfigs[$dbId]['connectionString'], $this->allConfigs[$dbId]['username'], $this->allConfigs[$dbId]['password'], $this->_attributes);

            if (!$instance)
                throw new CDbException(Mod::t('mod', 'CDbConnection failed to open the DB '.$dbId.' connection.'));

            $this->_pdos[$dbId]=$instance;
            return $this->_pdo =$instance;
        }
        return  $this->_pdo = $this->_pdos[$dbId];
	}
     ...
     * @param string $dbId default value: master
     * @return PDO the PDO instance, null if the connection is not established yet
     * @throws CException
     */
	public function getPdoInstance($dbId='master')
	{
        /*if($this->_pdos[$dbId]===null)
        {
            if(empty($this->allConfigs[$dbId]['connectionString']))
	...
	public function getSchema()
	{
		...
		else
		{
			...
			if(isset($this->driverMap[$driver]))
				return $this->_schema=Mod::createComponent($this->driverMap[$driver], $this);
			else
				throw new CDbException(Mod::t('mod','CDbConnection does not support reading schema for {driver} database.',
					array('{driver}'=>$driver)));
		}
	}
