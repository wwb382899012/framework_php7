...
class CDbCommand extends CComponent
{
	...
	public function prepare()
	{
		if($this->_statement==null)
		{
			...
			{
				Mod::log('Error in preparing SQL: '.$this->getText(),CLogger::LEVEL_ERROR,'system.db.CDbCommand');
				$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
				throw new CDbException(Mod::t('mod','CDbCommand failed to prepare the SQL statement: {error}',
	...
	public function execute($params=array())
	{
		...
		try
		{
			if($this->_connection->enableProfiling)
				Mod::beginProfile('system.db.CDbCommand.execute('.$this->getText().$par.')','system.db.CDbCommand.execute');
            ...
            for($retryCount=0; $retryCount<=$this->_connection->retryCount; ++$retryCount)
            {
                try
                {
                    ...
                    if($this->_connection->enableProfiling)
                        Mod::endProfile('system.db.CDbCommand.execute('.$this->getText().$par.')','system.db.CDbCommand.execute');
		...
		catch(Exception $e)
		{
			...
			/*$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
			$message=$e->getMessage();
			Mod::log(Mod::t('mod','CDbCommand::execute() failed: {error}. The SQL statement executed was: {sql}.',
				array('{error}'=>$message, '{sql}'=>$this->getText().$par)),CLogger::LEVEL_ERROR,'system.db.CDbCommand');

			if(MOD_DEBUG)
				$message.='. The SQL statement executed was: '.$this->getText().$par;*/

			throw new CDbException(Mod::t('mod','CDbCommand failed to execute the SQL statement: {error}',
	...
	private function queryInternal($method,$mode,$params=array())
	{
				...
				&& ($cache=Mod::app()->getComponent($this->_connection->queryCacheID))!==null)
		{
			...
			$cacheKey='mod:dbquery'.':'.$method.':'.$this->_connection->connectionString.':'.$this->_connection->username;
		...
		catch(Exception $e)
		{
			...
			$errorInfo=$e instanceof PDOException ? $e->errorInfo : null;
			$message=$e->getMessage();
			Mod::log(Mod::t('mod','CDbCommand::{method}() failed: {error}. The SQL statement executed was: {sql}.',
	...
	private function processConditions($conditions)
	{
		...
		throw new CDbException(Mod::t('mod', 'Unknown operator "{operator}".', array('{operator}'=>$operator)));
	}
