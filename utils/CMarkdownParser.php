...
require_once(Mod::getPathOfAlias('system.vendors.markdown.markdown').'.php');
if(!class_exists('HTMLPurifier_Bootstrap',false))
{
	require_once(Mod::getPathOfAlias('system.vendors.htmlpurifier').DIRECTORY_SEPARATOR.'HTMLPurifier.standalone.php');
...
class CMarkdownParser extends MarkdownExtra_Parser
{
	...
	public function safeTransform($content)
	{
		...
		$purifier->config->set('Cache.SerializerPath',Mod::app()->getRuntimePath());
	...
	{
		return Mod::getPathOfAlias('system.vendors.TextHighlighter.highlight').'.css';
	}
	...
	protected function createHighLighter($options)
	{
		...
		{
			require_once(Mod::getPathOfAlias('system.vendors.TextHighlighter.Text.Highlighter').'.php');
			require_once(Mod::getPathOfAlias('system.vendors.TextHighlighter.Text.Highlighter.Renderer.Html').'.php');
		}
