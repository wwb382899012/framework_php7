 ...
 * the Mod Wiki article
 * {@link http://www.modframework.com/wiki/425/use-crypt-for-password-storage/ Use crypt() for password storage}.
 * and the
...
class CPasswordHelper
{
	...
	protected static function checkBlowfish()
	{
		if(!function_exists('crypt'))
			throw new CException(Mod::t('mod','{class} requires the PHP crypt() function. This system does not have it.',
	...
	public static function hashPassword($password,$cost=13)
	{
		...
		if(!is_string($hash) || (function_exists('mb_strlen') ? mb_strlen($hash, '8bit') : strlen($hash))<32)
			throw new CException(Mod::t('mod','Internal error while generating hash.'));
	...
	{
		if(!is_numeric($cost))
			throw new CException(Mod::t('mod','{class}::$cost must be a number.',array('{class}'=>__CLASS__)));

		$cost=(int)$cost;
		if($cost<4 || $cost>31)
			throw new CException(Mod::t('mod','{class}::$cost must be between 4 and 31.',array('{class}'=>__CLASS__)));

		if(($random=Mod::app()->getSecurityManager()->generateRandomString(22,true))===false)
			if(($random=Mod::app()->getSecurityManager()->generateRandomString(22,false))===false)
				throw new CException(Mod::t('mod','Unable to generate random string.'));
