...
class CDateTimeParser
{
	...
	public static function parse($value,$pattern='MM/dd/yyyy',$defaults=array())
	{
		...
		$i=0;
		$n=self::$_mbstringAvailable ? mb_strlen($value,Mod::app()->charset) : strlen($value);
			...
			switch($token)
			{
				...
				case 'MMMM':
				{
					...
					if(($month=self::parseMonth($value,$i,'wide',$monthName))===false)
						return false;
					$i+=self::$_mbstringAvailable ? mb_strlen($monthName,Mod::app()->charset) : strlen($monthName);
					break;
				}
				case 'MMM':
				{
				...
				default:
				{
					$tn=self::$_mbstringAvailable ? mb_strlen($token,Mod::app()->charset) : strlen($token);
					if($i>=$n || ($token{0}!='?' && (self::$_mbstringAvailable ? mb_substr($value,$i,$tn,Mod::app()->charset) : substr($value,$i,$tn))!==$token))
						return false;
					$i+=$tn;
					break;
				}
	...
	{
		if(!($n=self::$_mbstringAvailable ? mb_strlen($pattern,Mod::app()->charset) : strlen($pattern)))
			return array();
		$tokens=array();
		$c0=self::$_mbstringAvailable ? mb_substr($pattern,0,1,Mod::app()->charset) : substr($pattern,0,1);

		for($start=0,$i=1;$i<$n;++$i)
		{
			$c=self::$_mbstringAvailable ? mb_substr($pattern,$i,1,Mod::app()->charset) : substr($pattern,$i,1);
			if($c!==$c0)
			{
				$tokens[]=self::$_mbstringAvailable ? mb_substr($pattern,$start,$i-$start,Mod::app()->charset) : substr($pattern,$start,$i-$start);
				$c0=$c;
				$start=$i;
			}
		}
		$tokens[]=self::$_mbstringAvailable ? mb_substr($pattern,$start,$n-$start,Mod::app()->charset) : substr($pattern,$start,$n-$start);
		return $tokens;
	}
	...
	protected static function parseInteger($value,$offset,$minLength,$maxLength)
	{
		...
		{
			$v=self::$_mbstringAvailable ? mb_substr($value,$offset,$len,Mod::app()->charset) : substr($value,$offset,$len);
			if(ctype_digit($v) && (self::$_mbstringAvailable ? mb_strlen($v,Mod::app()->charset) : strlen($v))>=$minLength)
				return $v;
		}
		return false;
	}
	...
	{
		$v=strtolower(self::$_mbstringAvailable ? mb_substr($value,$offset,2,Mod::app()->charset) : substr($value,$offset,2));
	...
	{
		$valueLength=self::$_mbstringAvailable ? mb_strlen($value,Mod::app()->charset) : strlen($value);
		for($len=1; $offset+$len<=$valueLength; $len++)
		{
			$monthName=self::$_mbstringAvailable ? mb_substr($value,$offset,$len,Mod::app()->charset) : substr($value,$offset,$len);
			if(!preg_match('/^[\p{L}\p{M}]+$/u',$monthName)) // unicode aware replacement for ctype_alpha($monthName)
			{
				$monthName=self::$_mbstringAvailable ? mb_substr($monthName,0,-1,Mod::app()->charset) : substr($monthName,0,-1);
				break;
			}
		}
		$monthName=self::$_mbstringAvailable ? mb_strtolower($monthName,Mod::app()->charset) : strtolower($monthName);

		$monthNames=Mod::app()->getLocale()->getMonthNames($width,false);
		foreach($monthNames as $k=>$v)
			$monthNames[$k]=rtrim(self::$_mbstringAvailable ? mb_strtolower($v,Mod::app()->charset) : strtolower($v),'.');

		$monthNamesStandAlone=Mod::app()->getLocale()->getMonthNames($width,true);
		foreach($monthNamesStandAlone as $k=>$v)
			$monthNamesStandAlone[$k]=rtrim(self::$_mbstringAvailable ? mb_strtolower($v,Mod::app()->charset) : strtolower($v),'.');
