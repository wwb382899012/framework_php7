 ...
 */
defined('MOD_ZII_PATH') or define('MOD_ZII_PATH',MOD_PATH.DIRECTORY_SEPARATOR.'zii');

/**
 * ModBase is a helper class serving common framework functionalities.
 *
 * Do not use ModBase directly. Instead, use its child class {@link Mod} where
 * you can customize methods of ModBase.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system
 * @since 1.0
...
class ModBase
{
	 ...
	 * If filter function returns true Mod autoloader will be skipped.
	 * @since 1.1.20
	 */
	 ...
	 * include path, or if you want to append additional autoloaders to the default Mod autoloader.
	 * @since 1.1.8
	 */
    ...
    /**
     * @var \system\components\di\Container the dependency injection (DI) container used by [[createObject()]].
     ...
     * @param null $config
     */
    public static function createContainer($config=null)
    {
        static::$container=new \system\components\di\Container();

        if(isset($config['di']))
        {
            if(is_array($config['di']))
            {
                foreach ($config['di'] as $k=>$v)
                {
                    if (is_string($v))
                        Mod::$container->set($k, $v);
                    elseif (is_array($v))
                    {
                        if ($v['singleton'])
                            Mod::$container->setSingleton($v['class'], $v['definition'], $v['params'] ?? []);
                        else
                            Mod::$container->set($v['class'], $v['definition'], $v['params'] ?? []);
                    }
                }
            }
            unset($config['di']);
     ...
     * @return mixed
     */
    public static function createMicroServiceApplication($config=null)
    {
        return self::createApplication('CMSApplication',$config);
    }
	 ...
	 * be either the class name or {@link ModBase::getPathOfAlias class path alias}.
	...
	public static function createComponent($config)
	{
        ...
        $object=static::$container->get($type,$params,$config);
	...
	public static function import($alias,$forceInclude=false)
	{
			...
			if(($path=self::getPathOfAlias($namespace))!==false)
			{
				...
				if($forceInclude)
				{
					if(is_file($classFile))
						...
						throw new CException(Mod::t('mod','Alias "{alias}" is invalid. Make sure it points to an existing PHP file and the file is readable.',array('{alias}'=>$alias)));
		...
		if(($pos=strrpos($alias,'.'))===false)  // a simple class name
		{
			...
			if($forceInclude && (Mod::autoload($alias,true) || class_exists($alias,true)))
				self::$_imports[$alias]=$alias;
			return $alias;
		}
	...
	public static function autoload($className,$classMapOnly=false)
	{
		...
		elseif(isset(self::$_coreClasses[$className]))
			include(MOD_PATH.self::$_coreClasses[$className]);
		elseif($classMapOnly)
			return false;
		else
		{
			...
			if(strpos($className,'\\')===false)  // class without namespace
			{
				if(self::$enableIncludePath===false)
				{
					foreach(self::$_includePaths as $path)
					{
						...
						{
							include($classFile);
							if(MOD_DEBUG && basename(realpath($classFile))!==$className.'.php')
								throw new CException(Mod::t('mod','Class name "{class}" does not match class file "{file}".', array(
									'{class}'=>$className,
									'{file}'=>$classFile,
								)));
							break;
	...
	{
		return Mod::t('mod','Powered by {mod}.', array('{mod}'=>'<a href="http://www.modframework.com/" rel="external">Mod Framework</a>'));
	}
	...
	public static function t($category,$message,$params=array(),$source=null,$language=null)
	{
		...
		{
			if($source===null)
				$source=($category==='mod'||$category==='zii')?'coreMessages':'messages';
	 ...
	 * when set to true. After this the Mod autoloader can not rely on loading classes via simple include anymore
	...
	public static function registerAutoloader($callback, $append=false)
	{
		...
		else
		{
			...
			spl_autoload_register($callback);
			spl_autoload_register(array('ModBase','autoload'));
		}
	}
	...
	private static $_coreClasses=array(
        ...
        //Custom add by youyi000 @2018-01-23
        'CUrl'=>'/components/curl/CUrl.php',
        'CWaeMailer'=>'/components/mailer/CWaeMailer.php',
        'Monitor'=>'/components/fluent/Monitor.php',
        'CTaskDaemonCommand'=>'/console/CTaskDaemonCommand.php',
        'ZSqlDataProvider' => '/components/grid/ZSqlDataProvider.php',
        'ZGridView' => '/components/grid/ZGridView.php',
        'ZPagination' => '/components/grid/ZPagination.php',
        'ZFormatter' => '/components/grid/ZFormatter.php',

        'CMSApplication' => '/micro/CMSApplication.php',
	);
}

spl_autoload_register(array('ModBase','autoload'));
require(MOD_PATH.'/base/interfaces.php');
