/**
 * jQuery Mod GridView plugin file.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @link http://www.modframework.com/
 * @copyright 2008-2010 Mod Software LLC
...
(function ($) {
	...
	methods = {
		...
		init: function (options) {
			...
			return this.each(function () {
				...
				if (settings.ajaxUpdate.length > 0) {
					$(document).on('click.modGridView', settings.updateSelector, function () {
						...
						if (settings.enableHistory && window.History.enabled) {
							...
							$('#' + id).modGridView('update', {url: $(this).attr('href')});
						}
						return false;
					});
				}

				$(document).on('change.modGridView keydown.modGridView', settings.filterSelector, function (event) {
					...
					if (settings.enableHistory && settings.ajaxUpdate !== false && window.History.enabled) {
						...
						$('#' + id).modGridView('update', {data: data});
					}
					return false;
				});
				...
				if (settings.enableHistory && settings.ajaxUpdate !== false && window.History.enabled) {
					$(window).bind('statechange', function() { // Note: We are using statechange instead of popstate
						...
						$('#' + id).modGridView('update', State.data);
					});
				}

				if (settings.selectableRows > 0) {
					selectCheckedRows(this.id);
					$(document).on('click.modGridView', '#' + id + ' .' + settings.tableClass + ' > tbody > tr', function (e) {
					...
					if (settings.selectableRows > 1) {
						$(document).on('click.modGridView', '#' + id + ' .select-on-check-all', function () {
							var $currentGrid = $('#' + id),
								$checks = $('input.select-on-check', $currentGrid),
					...
					$(document).on('click.modGridView', '#' + id + ' .select-on-check', false);
				}
			});
		},
		...
		update: function (options) {
			...
			return this.each(function () {
				...
				//class must be added after modXHR.abort otherwise ajax.error will remove it
	...
	$.fn.modGridView = function (method) {
		if (methods[method]) {
			return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
		} else if (typeof method === 'object' || !method) {
			return methods.init.apply(this, arguments);
		} else {
			$.error('Method ' + method + ' does not exist on jQuery.modGridView');
			return false;
		}
	};
...
/******************************************************************************
 *** DEPRECATED METHODS
 *** used before Mod 1.1.9
 ******************************************************************************/
	$.fn.modGridView.settings = gridSettings;
	/**
	 * Returns the key value for the specified row
	 ...
	 * @return string the key value
	 */
	$.fn.modGridView.getKey = function (id, row) {
		return $('#' + id).modGridView('getKey', row);
	};
	 ...
	 * @return string the URL that generates the grid view content.
	 */
	$.fn.modGridView.getUrl = function (id) {
		return $('#' + id).modGridView('getUrl');
	};
	 ...
	 * @return jQuery the jQuery collection of the cells in the specified row.
	 */
	$.fn.modGridView.getRow = function (id, row) {
		return $('#' + id).modGridView('getRow', row);
	};
	 ...
	 * @return jQuery the jQuery collection of the cells in the specified column.
	 */
	$.fn.modGridView.getColumn = function (id, column) {
		return $('#' + id).modGridView('getColumn', column);
	};
	 ...
	 * the URL to be requested is the one that generates the current content of the grid view.
	 */
	$.fn.modGridView.update = function (id, options) {
		$('#' + id).modGridView('update', options);
	};
	 ...
	 * @return array the key values of the currently selected rows.
	 */
	$.fn.modGridView.getSelection = function (id) {
		return $('#' + id).modGridView('getSelection');
	};
	 ...
	 * @return array the key values of the currently checked rows.
	 */
	$.fn.modGridView.getChecked = function (id, column_id) {
		return $('#' + id).modGridView('getChecked', column_id);
	};
})(jQuery);
