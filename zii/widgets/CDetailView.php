...
class CDetailView extends CWidget
{
	...
	{
		if($this->data===null)
			throw new CException(Mod::t('zii','Please specify the "data" property.'));
		if($this->attributes===null)
		{
			...
			elseif(is_array($this->data))
				...
				throw new CException(Mod::t('zii','Please specify the "attributes" property.'));
		}
		if($this->nullDisplay===null)
			$this->nullDisplay='<span class="null">'.Mod::t('zii','Not set').'</span>';
		...
		if($this->baseScriptUrl===null)
			$this->baseScriptUrl=Mod::app()->getAssetManager()->publish(Mod::getPathOfAlias('zii.widgets.assets')).'/detailview';
		...
		foreach($this->attributes as $attribute)
		{
			if(is_string($attribute))
			{
				if(!preg_match('/^([\w\.]+)(:(\w*))?(:(.*))?$/',$attribute,$matches))
					throw new CException(Mod::t('zii','The attribute must be specified in the format of "Name:Type:Label", where "Type" and "Label" are optional.'));
				$attribute=array(
					'name'=>$matches[1],
