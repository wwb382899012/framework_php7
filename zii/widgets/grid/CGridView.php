...
Mod::import('zii.widgets.CBaseListView');
Mod::import('zii.widgets.grid.CDataColumn');
Mod::import('zii.widgets.grid.CLinkColumn');
Mod::import('zii.widgets.grid.CButtonColumn');
Mod::import('zii.widgets.grid.CCheckBoxColumn');
...
class CGridView extends CBaseListView
{
	 ...
	 * In this function, you may use <code>$(gridID).modGridView('getSelection')</code> to get the key values
	 ...
	 * <code>$(gridID).modGridView('getSelection')</code> to retrieve the key values of the currently selected
	...
	public function init()
	{
		...
		if(empty($this->updateSelector))
			throw new CException(Mod::t('zii','The property updateSelector should be defined.'));
		if(empty($this->filterSelector))
			throw new CException(Mod::t('zii','The property filterSelector should be defined.'));

		if(!isset($this->htmlOptions['class']))
			$this->htmlOptions['class']='grid-view';

		if($this->baseScriptUrl===null)
			$this->baseScriptUrl=Mod::app()->getAssetManager()->publish(Mod::getPathOfAlias('zii.widgets.assets')).'/gridview';

		if($this->cssFile!==false)
		{
			if($this->cssFile===null)
				$this->cssFile=$this->baseScriptUrl.'/styles.css';
			Mod::app()->getClientScript()->registerCssFile($this->cssFile);
		}
	...
	protected function createDataColumn($text)
	{
		if(!preg_match('/^([\w\.]+)(:(\w*))?(:(.*))?$/',$text,$matches))
			throw new CException(Mod::t('zii','The column must be specified in the format of "Name:Type:Label", where "Type" and "Label" are optional.'));
	...
	public function registerClientScript()
	{
		...
		$cs->registerScriptFile($this->baseScriptUrl.'/jquery.modgridview.js',CClientScript::POS_END);
		$cs->registerScript(__CLASS__.'#'.$id,"jQuery('#$id').modGridView($options);");
	}
	...
	public function getFormatter()
	{
		if($this->_formatter===null)
			$this->_formatter=Mod::app()->format;
