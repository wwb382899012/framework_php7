...
Mod::import('zii.widgets.grid.CGridColumn');
...
class CButtonColumn extends CGridColumn
{
	 ...
	 */
	public $viewButtonUrl='Mod::app()->controller->createUrl("view",array("id"=>$data->primaryKey))';
	/**
	 ...
	 * please refer to the {@link http://www.php.net/manual/en/language.expressions.php php manual}.
	 */
	public $updateButtonUrl='Mod::app()->controller->createUrl("update",array("id"=>$data->primaryKey))';
	/**
	 ...
	 * please refer to the {@link http://www.php.net/manual/en/language.expressions.php php manual}.
	 */
	public $deleteButtonUrl='Mod::app()->controller->createUrl("delete",array("id"=>$data->primaryKey))';
	/**
	...
	protected function initDefaultButtons()
	{
		if($this->viewButtonLabel===null)
			$this->viewButtonLabel=Mod::t('zii','View');
		if($this->updateButtonLabel===null)
			$this->updateButtonLabel=Mod::t('zii','Update');
		if($this->deleteButtonLabel===null)
			$this->deleteButtonLabel=Mod::t('zii','Delete');
		...
		if($this->deleteConfirmation===null)
			$this->deleteConfirmation=Mod::t('zii','Are you sure you want to delete this item?');
		...
		if(!isset($this->buttons['delete']['click']))
		{
			...
			if(Mod::app()->request->enableCsrfValidation)
			{
				$csrfTokenName = Mod::app()->request->csrfTokenName;
				$csrfToken = Mod::app()->request->csrfToken;
...
function() {
	...
	jQuery('#{$this->grid->id}').modGridView('update', {
		type: 'POST',
...
EOD;
	...
	protected function registerClientScript()
	{
		...
		if($js!==array())
			Mod::app()->getClientScript()->registerScript(__CLASS__.'#'.$this->id, implode("\n",$js));
	}
