...
abstract class CBaseListView extends CWidget
{
	...
	public function init()
	{
		if($this->dataProvider===null)
			throw new CException(Mod::t('zii','The "dataProvider" property cannot be empty.'));
	...
	public function renderKeys()
	{
		echo CHtml::openTag('div',array(
			'class'=>'keys',
			'style'=>'display:none',
			'title'=>Mod::app()->getRequest()->getUrl(),
		));
	...
	public function renderSummary()
	{
		...
		if($this->enablePagination)
		{
			...
			if(($summaryText=$this->summaryText)===null)
				$summaryText=Mod::t('zii','Displaying {start}-{end} of 1 result.|Displaying {start}-{end} of {count} results.',$total);
		...
		{
			if(($summaryText=$this->summaryText)===null)
				$summaryText=Mod::t('zii','Total 1 result.|Total {count} results.',$count);
			echo strtr($summaryText,array(
				'{count}'=>$count,
				'{start}'=>1,
				'{end}'=>$count,
