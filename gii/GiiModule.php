...
Mod::import('system.gii.CCodeGenerator');
Mod::import('system.gii.CCodeModel');
Mod::import('system.gii.CCodeFile');
Mod::import('system.gii.CCodeForm');
...
class GiiModule extends CWebModule
{
	...
	{
		parent::init();
		Mod::setPathOfAlias('gii',dirname(__FILE__));
		Mod::app()->setComponents(array(
			'errorHandler'=>array(
				'class'=>'CErrorHandler',
			...
			'user'=>array(
				'class'=>'CWebUser',
				'stateKeyPrefix'=>'gii',
				'loginUrl'=>Mod::app()->createUrl($this->getId().'/default/login'),
			),
			'widgetFactory' => array(
	...
	public function getAssetsUrl()
	{
		if($this->_assetsUrl===null)
			$this->_assetsUrl=Mod::app()->getAssetManager()->publish(Mod::getPathOfAlias('gii.assets'));
	...
	public function beforeControllerAction($controller, $action)
	{
		if(parent::beforeControllerAction($controller, $action))
		{
			...
			if(!$this->allowIp(Mod::app()->request->userHostAddress) && $route!=='default/error')
			...
			if($this->password!==false && Mod::app()->user->isGuest && !in_array($route,$publicPages))
				Mod::app()->user->loginRequired();
			else
				return true;
		}
		return false;
