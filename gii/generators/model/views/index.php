...
Mod::app()->clientScript->registerScript('gii.model',"
$('#{$class}_connectionId').change(function(){
	var tableName=$('#{$class}_tableName');
	tableName.autocomplete('option', 'source', []);
	$.ajax({
		url: '".Mod::app()->getUrlManager()->createUrl('gii/model/getTableNames')."',
		data: {db: this.value},
		dataType: 'json'
	}).done(function(data){
...
<?php $form=$this->beginWidget('CCodeForm', array('model'=>$model)); ?>
	...
	<div class="row">
		...
		<?php $this->widget('zii.widgets.jui.CJuiAutoComplete',array(
			...
			'attribute'=>'tableName',
			'name'=>'tableName',
			'source'=>Mod::app()->hasComponent($model->connectionId) ? array_keys(Mod::app()->{$model->connectionId}->schema->getTables()) : array(),
			'options'=>array(
				'minLength'=>'0',
