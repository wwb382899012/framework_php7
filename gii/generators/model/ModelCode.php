...
class ModelCode extends CCodeModel
{
	...
	public function prepare()
	{
		...
		{
			$tables=Mod::app()->{$this->connectionId}->schema->getTables($schema);
			if($this->tablePrefix!='')
			{
		...
		foreach($tables as $table)
		{
			...
			$this->files[]=new CCodeFile(
				Mod::getPathOfAlias($this->modelPath).'/'.$className.'.php',
	...
	{
		$connection=Mod::app()->{$this->connectionId};
	...
	{
		if($addBrackets && Mod::app()->{$this->connectionId}->tablePrefix=='')
			return $tableName;
		$prefix=$this->tablePrefix!='' ? $this->tablePrefix : Mod::app()->{$this->connectionId}->tablePrefix;
		if($prefix!='')
		{
			if($addBrackets && Mod::app()->{$this->connectionId}->tablePrefix!='')
			{
				$prefix=Mod::app()->{$this->connectionId}->tablePrefix;
				$lb='{{';
				$rb='}}';
			}
			else
	...
	protected function generateRelations()
	{
		...
		$relations=array();
		foreach(Mod::app()->{$this->connectionId}->schema->getTables($schemaName) as $table)
		{
			if($this->tablePrefix!='' && strpos($table->name,$this->tablePrefix)!==0)
				continue;
			$tableName=$table->name;
	...
	protected function generateRelationName($tableName, $fkName, $multiple)
	{
		...
		$rawName=$name;
		$table=Mod::app()->{$this->connectionId}->schema->getTable($tableName);
		$i=0;
	...
	{
		if(Mod::app()->hasComponent($this->connectionId)===false || !(Mod::app()->getComponent($this->connectionId) instanceof CDbConnection))
