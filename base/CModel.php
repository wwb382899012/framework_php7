...
abstract class CModel extends CComponent implements IteratorAggregate, ArrayAccess
{
	...
	public function createValidators()
	{
		...
		foreach($this->rules() as $rule)
		{
			if(isset($rule[0],$rule[1]))  // attributes, validator name
				...
				throw new CException(Mod::t('mod','{class} has an invalid validation rule. The rule must specify attributes to be validated and the validator name.',
	 ...
	 * The default implementation will log a warning message if MOD_DEBUG is on.
	...
	{
		if(MOD_DEBUG)
			Mod::log(Mod::t('mod','Failed to set unsafe attribute "{attribute}" of "{class}".',array('{attribute}'=>$name, '{class}'=>get_class($this))),CLogger::LEVEL_WARNING);
	}
