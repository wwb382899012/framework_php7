 ...
 * (i.e. MOD_DEBUG is defined as true). Detailed error information with source code
...
class CErrorHandler extends CApplicationComponent
{
	 ...
	 * Inside the action, it can retrieve the error information by Mod::app()->errorHandler->error.
	...
	protected function handleException($exception)
	{
		...
		if($app instanceof CWebApplication)
		{
			...
			if(!headers_sent())
			{
				$httpVersion=Mod::app()->request->getHttpVersion();
	...
	protected function renderException()
	{
		...
		if($exception instanceof CHttpException || !MOD_DEBUG)
			$this->renderError();
		else
		{
			if($this->isAjaxRequest())
				Mod::app()->displayException($exception);
			else
	...
	protected function renderError()
	{
		if($this->errorAction!==null)
			Mod::app()->runController($this->errorAction);
		else
		{
			$data=$this->getError();
			if($this->isAjaxRequest())
				Mod::app()->displayError($data['code'],$data['message'],$data['file'],$data['line']);
			elseif(MOD_DEBUG)
	...
	protected function getViewFile($view,$code)
	{
		$viewPaths=array(
			...
			MOD_PATH.DIRECTORY_SEPARATOR.'views',
		);
	...
	protected function getVersionInfo()
	{
		if(MOD_DEBUG)
		{
			$version='<a href="http://www.modframework.com/">Mod Framework</a>/'.Mod::getVersion();
