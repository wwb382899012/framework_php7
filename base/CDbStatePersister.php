...
class CDbStatePersister extends CApplicationComponent implements IStatePersister
{
	...
	public function init()
	{
		...
		if($this->stateTableName===null)
			throw new CException(Mod::t('mod', 'stateTableName param cannot be null.'));
		$this->db=Mod::app()->getComponent($this->dbComponent);
		if($this->db===null)
			throw new CException(Mod::t('mod', '\'{db}\' component doesn\'t exist.',array(
				'{db}'=>$this->dbComponent
			)));
		if(!($this->db instanceof CDbConnection))
			throw new CException(Mod::t ('mod', '\'{db}\' component is not a valid CDbConnection instance.',array(
				'{db}'=>$this->dbComponent
			)));
	...
	protected function createTable()
	{
		...
		{
			throw new CException(Mod::t('mod','Can\'t create state persister table. Check CREATE privilege for \'{db}\' connection user or create table manually with SQL: {sql}.',array('{db}'=>$this->dbComponent,'{sql}'=>$command->text ) ) );
		}
	}
}
