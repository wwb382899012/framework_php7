...
class CComponent
{
	...
	public function __get($name)
	{
		...
		throw new CException(Mod::t('mod','Property "{class}.{property}" is not defined.',
	...
	public function __set($name,$value)
	{
		...
		if(method_exists($this,'get'.$name))
			throw new CException(Mod::t('mod','Property "{class}.{property}" is read only.',
				array('{class}'=>get_class($this), '{property}'=>$name)));
		else
			throw new CException(Mod::t('mod','Property "{class}.{property}" is not defined.',
				array('{class}'=>get_class($this), '{property}'=>$name)));
	...
	public function __call($name,$parameters)
	{
		...
		throw new CException(Mod::t('mod','{class} and its behaviors do not have a method or closure named "{name}".',
	 ...
	 * parameter to {@link ModBase::createComponent} to create the behavior object.
	...
	public function attachBehavior($name,$behavior)
	{
		if(!($behavior instanceof IBehavior))
			$behavior=Mod::createComponent($behavior);
	...
	public function getEventHandlers($name)
	{
		if($this->hasEvent($name))
		{
			...
			throw new CException(Mod::t('mod','Event "{class}.{event}" is not defined.',
	...
	public function raiseEvent($name,$event)
	{
				...
				elseif(is_callable($handler,true))
				{
					if(is_array($handler))
					{
						...
						elseif(method_exists($object,$method))
							...
							throw new CException(Mod::t('mod','Event "{class}.{event}" is attached with an invalid handler "{handler}".',
		...
		elseif(MOD_DEBUG && !$this->hasEvent($name))
			throw new CException(Mod::t('mod','Event "{class}.{event}" is not defined.',
				array('{class}'=>get_class($this), '{event}'=>$name)));
	}
