...
abstract class CModule extends CComponent
{
	...
	public function __construct($id,$parent,$config=null)
	{
		...
		Mod::setPathOfAlias($id,$this->getBasePath());
	...
	public function setBasePath($path)
	{
		if(($this->_basePath=realpath($path))===false || !is_dir($this->_basePath))
			throw new CException(Mod::t('mod','Base path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
	...
	public function setModulePath($value)
	{
		if(($this->_modulePath=realpath($value))===false || !is_dir($this->_modulePath))
			throw new CException(Mod::t('mod','The module path "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function setAliases($mappings)
	{
		...
		{
			if(($path=Mod::getPathOfAlias($alias))!==false)
				Mod::setPathOfAlias($name,$path);
			else
				Mod::setPathOfAlias($name,$alias);
		}
	}
	...
	public function getModule($id)
	{
		...
		elseif(isset($this->_moduleConfig[$id]))
		{
			...
			{
				Mod::trace("Loading \"$id\" module",'system.base.CModule');
				$class=$config['class'];
				unset($config['class'], $config['enabled']);
				if($this===Mod::app())
					$module=Mod::createComponent($class,$id,null,$config);
				else
					$module=Mod::createComponent($class,$this->getId().'/'.$id,$this,$config);
	...
	public function setModules($modules,$merge=true)
	{
		foreach($modules as $id=>$module)
		{
			...
			else
			{
				...
				{
					if (Mod::getPathOfAlias($id)===false)
						Mod::setPathOfAlias($id,$this->getModulePath().DIRECTORY_SEPARATOR.$id);
	...
	public function getComponent($id,$createIfNull=true)
	{
		...
		elseif(isset($this->_componentConfig[$id]) && $createIfNull)
		{
			...
			if(!isset($config['enabled']) || $config['enabled'])
			{
				Mod::trace("Loading \"$id\" application component",'system.CModule');
				unset($config['enabled']);
				$component=Mod::createComponent($config);
				$component->init();
	...
	public function setComponent($id,$component,$merge=true)
	{
		...
		elseif($component instanceof IApplicationComponent)
		{
			...
			if(!$component->getIsInitialized())
				$component->init();
            //Mod::$container->setSingleton($id,$component);
			return;
		}
		elseif(isset($this->_components[$id]))
		{
            ...
            /*if(isset($component['class']))
                Mod::$container->set($id,$component);*/
	 ...
	 *         'class'=>'CDbCache',
	 *         'connectionID'=>'db',
	 *         'enabled'=>!MOD_DEBUG,  // enable caching in non-debug mode
	 *     ),
	 * )
	 * </pre>
	 *
