...
class CStatePersister extends CApplicationComponent implements IStatePersister
{
	...
	public function init()
	{
		...
		if($this->stateFile===null)
			$this->stateFile=Mod::app()->getRuntimePath().DIRECTORY_SEPARATOR.'state.bin';
		$dir=dirname($this->stateFile);
		if(!is_dir($dir) || !is_writable($dir))
			throw new CException(Mod::t('mod','Unable to create application state file "{file}". Make sure the directory containing the file exists and is writable by the Web server process.',
	...
	public function load()
	{
		...
		{
			$cacheKey='Mod.CStatePersister.'.$stateFile;
