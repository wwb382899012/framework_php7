 ...
 *   Mod framework messages. This application component is dynamically loaded when needed.</li>
...
abstract class CApplication extends CModule
{
	...
	public function __construct($config=null)
	{
		...
		Mod::setPathOfAlias('application',$this->getBasePath());
        Mod::setPathOfAlias('app',$this->getBasePath());
		Mod::setPathOfAlias('webroot',dirname($_SERVER['SCRIPT_FILENAME']));
		if(isset($config['extensionPath']))
		{
			...
			Mod::setPathOfAlias('ext',$this->getBasePath().DIRECTORY_SEPARATOR.'extensions');
	...
	public function setBasePath($path)
	{
		if(($this->_basePath=realpath($path))===false || !is_dir($this->_basePath))
			throw new CException(Mod::t('mod','Application base path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
	...
	public function setRuntimePath($path)
	{
		if(($runtimePath=realpath($path))===false || !is_dir($runtimePath) || !is_writable($runtimePath))
			throw new CException(Mod::t('mod','Application runtime path "{path}" is not valid. Please make sure it is a directory writable by the Web server process.',
				array('{path}'=>$path)));
	...
	public function setExtensionPath($path)
	{
		if(($extensionPath=realpath($path))===false || !is_dir($extensionPath))
			throw new CException(Mod::t('mod','Extension path "{path}" does not exist.',
				array('{path}'=>$path)));
		Mod::setPathOfAlias('ext',$extensionPath);
	}
	...
	public function getLocaleDataPath()
	{
		...
		if(empty($vars['dataPath']))
			return Mod::getPathOfAlias('system.i18n.data');
	 ...
	 * that constant MOD_ENABLE_EXCEPTION_HANDLER be defined true.
	 *
	...
	public function handleException($exception)
	{
		...
		$message.="\n---";
		Mod::log($message,CLogger::LEVEL_ERROR,$category);
	 ...
	 * that constant MOD_ENABLE_ERROR_HANDLER be defined true.
	 *
	...
	public function handleError($code,$message,$file,$line)
	{
		if($code & error_reporting())
		{
			...
			Mod::log($log,CLogger::LEVEL_ERROR,'php');

			try
			{
				Mod::import('CErrorEvent',true);
	...
	{
		if(MOD_ENABLE_EXCEPTION_HANDLER)
			set_exception_handler(array($this,'handleException'));
		if(MOD_ENABLE_ERROR_HANDLER)
	...
	protected function registerCoreComponents()
	{
		$components=array(
			'coreMessages'=>array(
				...
				'language'=>'en_us',
				'basePath'=>MOD_PATH.DIRECTORY_SEPARATOR.'messages',
			),
			'db'=>array(
				'class'=>'CDbConnection',
			),
