 ...
 *
 * CSecurityManager is used by Mod components and applications for security-related purpose.
...
{
	const STATE_VALIDATION_KEY='Mod.CSecurityManager.validationkey';
	const STATE_ENCRYPTION_KEY='Mod.CSecurityManager.encryptionkey';
	...
	public function setValidationKey($value)
	{
		if(!empty($value))
			...
			throw new CException(Mod::t('mod','CSecurityManager.validationKey cannot be empty.'));
	}
	...
	public function getEncryptionKey()
	{
		...
		{
			if(($key=Mod::app()->getGlobalState(self::STATE_ENCRYPTION_KEY))!==null)
				...
				if(($key=$this->generateRandomString(32,true))===false)
					if(($key=$this->generateRandomString(32,false))===false)
						throw new CException(Mod::t('mod',
							'CSecurityManager::generateRandomString() cannot generate random string in the current environment.'));
				$this->setEncryptionKey($key);
				Mod::app()->setGlobalState(self::STATE_ENCRYPTION_KEY,$key);
			}
			return $this->_encryptionKey;
		}
	}
	...
	protected function openCryptModule()
	{
		if(extension_loaded('mcrypt'))
		{
			...
			if($module===false)
				throw new CException(Mod::t('mod','Failed to initialize the mcrypt module.'));

			return $module;
		}
		else
			throw new CException(Mod::t('mod','CSecurityManager requires PHP mcrypt extension to be loaded in order to use data encryption feature.'));
	}
	...
	public function computeHMAC($data,$key=null,$hashAlgorithm=null)
	{
		...
		{
			throw new CException(Mod::t('mod','Only SHA1 and MD5 hashing algorithms are supported when using PHP 5.1.1 or below.'));
		}
		...
		if(is_string($key))
		{
			...
			if($supportedKeyLengths)
			{
				if(!in_array($this->strlen($key),$supportedKeyLengths)) {
					throw new CException(Mod::t('mod','Encryption key length can be {keyLengths}.',array('{keyLengths}'=>implode(',',$supportedKeyLengths))));
				}
			}
			elseif(isset(self::$encryptionKeyMinimumLengths[$cryptAlgorithm]))
			{
				...
				if($this->strlen($key)<$minLength || $this->strlen($key)>$maxLength)
					throw new CException(Mod::t('mod','Encryption key length must be between {minLength} and {maxLength}.',array('{minLength}'=>$minLength,'{maxLength}'=>$maxLength)));
			}
			else
				throw new CException(Mod::t('mod','Failed to validate key. Supported key lengths of cipher not known.'));
		}
		else
			throw new CException(Mod::t('mod','Encryption key should be a string.'));
	}
	...
	public function legacyDecrypt($data,$key=null,$cipher='des')
	{
		if (!$key)
		{
			$key=Mod::app()->getGlobalState(self::STATE_ENCRYPTION_KEY);
			if(!$key)
				throw new CException(Mod::t('mod','No encryption key specified.'));
			$key = md5($key);
		}
