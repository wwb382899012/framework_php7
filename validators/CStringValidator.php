...
class CStringValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		if(is_array($value))
		{
			// https://github.com/modsoft/mod/issues/1955
			$this->addError($object,$attribute,Mod::t('mod','{attribute} is invalid.'));
			return;
		}

		if(function_exists('mb_strlen') && $this->encoding!==false)
			$length=mb_strlen($value, $this->encoding ? $this->encoding : Mod::app()->charset);
		else
			$length=strlen($value);

		if($this->min!==null && $length<$this->min)
		{
			$message=$this->tooShort!==null?$this->tooShort:Mod::t('mod','{attribute} is too short (minimum is {min} characters).');
			$this->addError($object,$attribute,$message,array('{min}'=>$this->min));
		}
		if($this->max!==null && $length>$this->max)
		{
			$message=$this->tooLong!==null?$this->tooLong:Mod::t('mod','{attribute} is too long (maximum is {max} characters).');
			$this->addError($object,$attribute,$message,array('{max}'=>$this->max));
		}
		if($this->is!==null && $length!==$this->is)
		{
			$message=$this->message!==null?$this->message:Mod::t('mod','{attribute} is of the wrong length (should be {length} characters).');
	...
	public function clientValidateAttribute($object,$attribute)
	{
		...
		if(($message=$this->message)===null)
			$message=Mod::t('mod','{attribute} is of the wrong length (should be {length} characters).');
		...
		if(($tooShort=$this->tooShort)===null)
			$tooShort=Mod::t('mod','{attribute} is too short (minimum is {min} characters).');
		...
		if(($tooLong=$this->tooLong)===null)
			$tooLong=Mod::t('mod','{attribute} is too long (maximum is {max} characters).');
