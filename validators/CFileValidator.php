...
class CFileValidator extends CValidator
{
	...
	protected function validateAttribute($object, $attribute)
	{
		...
		if($this->maxFiles > 1)
		{
			...
			{
				$message=$this->tooMany!==null?$this->tooMany : Mod::t('mod', '{attribute} cannot accept more than {limit} files.');
	...
	protected function validateFile($object, $attribute, $file)
	{
		...
		{
			$message=$this->tooLarge!==null?$this->tooLarge : Mod::t('mod','The file "{file}" is too large. Its size cannot exceed {limit} bytes.');
		...
		elseif($error!==UPLOAD_ERR_OK)
		{
			...
			elseif($error==UPLOAD_ERR_PARTIAL)
				throw new CException(Mod::t('mod','The file "{file}" was only partially uploaded.',array('{file}'=>$file->getName())));
			elseif($error==UPLOAD_ERR_NO_TMP_DIR)
				throw new CException(Mod::t('mod','Missing the temporary folder to store the uploaded file "{file}".',array('{file}'=>$file->getName())));
			elseif($error==UPLOAD_ERR_CANT_WRITE)
				throw new CException(Mod::t('mod','Failed to write the uploaded file "{file}" to disk.',array('{file}'=>$file->getName())));
			elseif(defined('UPLOAD_ERR_EXTENSION') && $error==UPLOAD_ERR_EXTENSION)  // available for PHP 5.2.0 or above
				throw new CException(Mod::t('mod','A PHP extension stopped the file upload.'));
			else
				throw new CException(Mod::t('mod','Unable to upload the file "{file}" because of an unrecognized error.',array('{file}'=>$file->getName())));
		}

		if($this->minSize!==null && $file->getSize()<$this->minSize)
		{
			$message=$this->tooSmall!==null?$this->tooSmall : Mod::t('mod','The file "{file}" is too small. Its size cannot be smaller than {limit} bytes.');
		...
		if($this->types!==null)
		{
			...
			{
				$message=$this->wrongType!==null?$this->wrongType : Mod::t('mod','The file "{file}" cannot be uploaded. Only files with these extensions are allowed: {extensions}.');
		...
		if($this->mimeTypes!==null && !empty($file->tempName))
		{
			...
			elseif(function_exists('mime_content_type'))
				...
				throw new CException(Mod::t('mod','In order to use MIME-type validation provided by CFileValidator fileinfo PECL extension should be installed.'));
			...
			{
				$message=$this->wrongMimeType!==null?$this->wrongMimeType : Mod::t('mod','The file "{file}" cannot be uploaded. Only files of these MIME-types are allowed: {mimeTypes}.');
	...
	protected function emptyAttribute($object, $attribute)
	{
		...
		if(!$this->allowEmpty)
		{
			$message=$this->message!==null?$this->message : Mod::t('mod','{attribute} cannot be blank.');
