...
class CUniqueValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		$className=$this->className===null?get_class($object):Mod::import($this->className);
		$attributeName=$this->attributeName===null?$attribute:$this->attributeName;
		$finder=$this->getModel($className);
		$table=$finder->getTableSchema();
		if(($column=$table->getColumn($attributeName))===null)
			throw new CException(Mod::t('mod','Table "{table}" does not have a column named "{column}".',
		...
		if($exists)
		{
			$message=$this->message!==null?$this->message:Mod::t('mod','{attribute} "{value}" has already been taken.');
