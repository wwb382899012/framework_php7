...
class CCaptchaValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		// reason of array checking is explained here: https://github.com/modsoft/mod/issues/1955
		if(is_array($value) || !$captcha->validate($value,$this->caseSensitive))
		{
			$message=$this->message!==null?$this->message:Mod::t('mod','The verification code is incorrect.');
	...
	{
		if(($captcha=Mod::app()->getController()->createAction($this->captchaAction))===null)
		{
			if(strpos($this->captchaAction,'/')!==false) // contains controller or module
			{
				if(($ca=Mod::app()->createController($this->captchaAction))!==null)
				{
			...
			if($captcha===null)
				throw new CException(Mod::t('mod','CCaptchaValidator.action "{id}" is invalid. Unable to find such an action in the current controller.',
