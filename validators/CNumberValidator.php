...
class CNumberValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		if(!is_numeric($value))
		{
			...
			// https://github.com/modsoft/mod/issues/1669
			$message=$this->message!==null?$this->message:Mod::t('mod','{attribute} must be a number.');
		...
		if($this->integerOnly)
		{
			...
			{
				$message=$this->message!==null?$this->message:Mod::t('mod','{attribute} must be an integer.');
				$this->addError($object,$attribute,$message);
			}
		}
		else
		...
		{
			$message=$this->tooSmall!==null?$this->tooSmall:Mod::t('mod','{attribute} is too small (minimum is {min}).');
			$this->addError($object,$attribute,$message,array('{min}'=>$this->min));
		}
		if($this->max!==null && $value>$this->max)
		{
			$message=$this->tooBig!==null?$this->tooBig:Mod::t('mod','{attribute} is too big (maximum is {max}).');
	...
	public function clientValidateAttribute($object,$attribute)
	{
		...
		if(($message=$this->message)===null)
			$message=$this->integerOnly ? Mod::t('mod','{attribute} must be an integer.') : Mod::t('mod','{attribute} must be a number.');
		$message=strtr($message, array(
			'{attribute}'=>$label,
		));

		if(($tooBig=$this->tooBig)===null)
			$tooBig=Mod::t('mod','{attribute} is too big (maximum is {max}).');
		...
		if(($tooSmall=$this->tooSmall)===null)
			$tooSmall=Mod::t('mod','{attribute} is too small (minimum is {min}).');
