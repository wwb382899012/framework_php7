...
class CDateFormatter extends CComponent
{
	 ...
	 * @return string formatted date time. Null if $time is null. (the null value check is available since Mod 1.1.11)
	 */
	...
	protected function formatMonth($pattern,$date)
	{
		...
		switch($pattern)
		{
			...
			default:
				throw new CException(Mod::t('mod','The pattern for month must be "M", "MM", "MMM", "MMMM", "L", "LL", "LLL" or "LLLL".'));
		}
	}
	...
	protected function formatDay($pattern,$date)
	{
		...
		elseif($pattern==='dd')
			...
			throw new CException(Mod::t('mod','The pattern for day of the month must be "d" or "dd".'));
	}
	...
	protected function formatDayInYear($pattern,$date)
	{
		...
		if(($n=strlen($pattern))<=3)
			...
			throw new CException(Mod::t('mod','The pattern for day in year must be "D", "DD" or "DDD".'));
	}
	...
	protected function formatDayInMonth($pattern,$date)
	{
		if($pattern==='F')
			...
			throw new CException(Mod::t('mod','The pattern for day in month must be "F".'));
	}
	...
	protected function formatDayInWeek($pattern,$date)
	{
		...
		switch($pattern)
		{
			...
			default:
				throw new CException(Mod::t('mod','The pattern for day of the week must be "E", "EE", "EEE", "EEEE", "EEEEE", "e", "ee", "eee", "eeee", "eeeee", "c", "cccc" or "ccccc".'));
		}
	}
	...
	protected function formatPeriod($pattern,$date)
	{
		if($pattern==='a')
		{
			...
			throw new CException(Mod::t('mod','The pattern for AM/PM marker must be "a".'));
	}
	...
	protected function formatHour24($pattern,$date)
	{
		...
		elseif($pattern==='HH')
			...
			throw new CException(Mod::t('mod','The pattern for 24 hour format must be "H" or "HH".'));
	}
	...
	protected function formatHour12($pattern,$date)
	{
		...
		elseif($pattern==='hh')
			...
			throw new CException(Mod::t('mod','The pattern for 12 hour format must be "h" or "hh".'));
	}
	...
	protected function formatHourInDay($pattern,$date)
	{
		...
		elseif($pattern==='kk')
			...
			throw new CException(Mod::t('mod','The pattern for hour in day must be "k" or "kk".'));
	}
	...
	protected function formatHourInPeriod($pattern,$date)
	{
		...
		elseif($pattern==='KK')
			...
			throw new CException(Mod::t('mod','The pattern for hour in AM/PM must be "K" or "KK".'));
	}
	...
	protected function formatMinutes($pattern,$date)
	{
		...
		elseif($pattern==='mm')
			...
			throw new CException(Mod::t('mod','The pattern for minutes must be "m" or "mm".'));
	}
	...
	protected function formatSeconds($pattern,$date)
	{
		...
		elseif($pattern==='ss')
			...
			throw new CException(Mod::t('mod','The pattern for seconds must be "s" or "ss".'));
	}
	...
	protected function formatWeekInYear($pattern,$date)
	{
		if($pattern==='w')
			...
			throw new CException(Mod::t('mod','The pattern for week in year must be "w".'));
	}
	...
	protected function formatWeekInMonth($pattern,$date)
	{
		if($pattern==='W')
		{
			...
			throw new CException(Mod::t('mod','The pattern for week in month must be "W".'));
	}
	...
	protected function formatTimeZone($pattern,$date)
	{
		...
		elseif($pattern[0]==='Z')
			...
			throw new CException(Mod::t('mod','The pattern for time zone must be "z" or "v".'));
	}
	...
	protected function formatEra($pattern,$date)
	{
		...
		switch($pattern)
		{
			...
			default:
				throw new CException(Mod::t('mod','The pattern for era must be "G", "GG", "GGG", "GGGG" or "GGGGG".'));
		}
	}
}
