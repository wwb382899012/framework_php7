 ...
 * the directory that contains the extension class file. When using Mod::t() to translate an extension message,
...
{
	const CACHE_KEY_PREFIX='Mod.CPhpMessageSource.';
	 ...
	 * When using Mod::t() to translate an extension message, the category name should be
	...
	public function init()
	{
		...
		if($this->basePath===null)
			$this->basePath=Mod::getPathOfAlias('application.messages');
	}
		...
		if(!isset($this->_files[$category][$language]))
		{
			if(($pos=strpos($category,'.'))!==false)
			{
				...
				if(isset($this->extensionPaths[$extensionClass]))
					$this->_files[$category][$language]=Mod::getPathOfAlias($this->extensionPaths[$extensionClass]).DIRECTORY_SEPARATOR.$language.DIRECTORY_SEPARATOR.$extensionCategory.'.php';
				else
				{
	...
	protected function loadMessages($category,$language)
	{
		...
		if($this->cachingDuration>0 && $this->cacheID!==false && ($cache=Mod::app()->getComponent($this->cacheID))!==null)
		{
