...
class CList extends CComponent implements IteratorAggregate,ArrayAccess,Countable
{
	...
	public function itemAt($index)
	{
		...
		elseif($index>=0 && $index<$this->_c) // in case the value is null
			...
			throw new CException(Mod::t('mod','List index "{index}" is out of bound.',
				array('{index}'=>$index)));
	}
	...
	public function insertAt($index,$item)
	{
		if(!$this->_r)
		{
			...
			throw new CException(Mod::t('mod','The list is read only.'));
	}
	...
	public function copyFrom($data)
	{
		...
		elseif($data!==null)
			throw new CException(Mod::t('mod','List data must be an array or an object implementing Traversable.'));
	}
