...
class CWebService extends CComponent
{
	 ...
	 * to {@link Mod::createComponent} to create the generator object. Default value is 'CWsdlGenerator'.
	 * @since 1.1.12
	 */
	...
	{
		$providerClass=is_object($this->provider) ? get_class($this->provider) : Mod::import($this->provider,true);
		if($this->wsdlCacheDuration>0 && $this->cacheID!==false && ($cache=Mod::app()->getComponent($this->cacheID))!==null)
		{
			$key='Mod.CWebService.'.$providerClass.$this->serviceUrl.$this->encoding;
			if(($wsdl=$cache->get($key))!==false)
				return $wsdl;
		}
		$generator=Mod::createComponent($this->generatorConfig);
	...
	public function run()
	{
		...
		Mod::app()->attachEventHandler('onError',array($this,'handleError'));
		try
		{
			if($this->persistence!==null)
				$server->setPersistence($this->persistence);
			if(is_string($this->provider))
				$provider=Mod::createComponent($this->provider);
			else
		...
		catch(Exception $e)
		{
			if($e->getCode()!==self::SOAP_ERROR) // non-PHP error
			{
				...
				Mod::log($e->__toString(),CLogger::LEVEL_ERROR,'application');
			}
			...
			Mod::app()->onEndRequest(new CEvent($this));
	...
	protected function getOptions()
	{
		...
		{
			$className=Mod::import($className,true);
			if(is_int($type))
				$type=$className;
