...
class CHtml
{
	...
	{
		return htmlspecialchars($text,ENT_QUOTES,Mod::app()->charset);
	}
	...
	public static function encodeArray($data)
	{
		...
		{
			if(is_string($key))
				$key=htmlspecialchars($key,ENT_QUOTES,Mod::app()->charset);
			if(is_string($value))
				$value=htmlspecialchars($value,ENT_QUOTES,Mod::app()->charset);
	...
	public static function refresh($seconds,$url='')
	{
		...
		Mod::app()->clientScript->registerMetaTag($content,null,'refresh');
	}
...
EOD;
	...
	{
		Mod::app()->getClientScript()->registerCoreScript('jquery');
	...
	{
		return Mod::app()->getAssetManager()->publish($path,$hashByName);
	}
	...
	public static function normalizeUrl($url)
	{
		if(is_array($url))
		{
			if(isset($url[0]))
			{
				if(($c=Mod::app()->getController())!==null)
					$url=$c->createUrl($url[0],array_splice($url,1));
				else
					$url=Mod::app()->createUrl($url[0],array_splice($url,1));
			}
			else
				$url='';
		}
		return $url==='' ? Mod::app()->getRequest()->getUrl() : $url;
	}
	...
	public static function errorSummary($model,$header=null,$footer=null,$htmlOptions=array())
	{
		...
		{
			if($header===null)
				$header='<p>'.Mod::t('mod','Please fix the following input errors:').'</p>';
			if(!isset($htmlOptions['class']))
	...
	public static function setModelNameConverter($converter)
	{
		...
		elseif($converter===null)
			...
			throw new CException(Mod::t('mod','The $converter argument must be a valid callback or null.'));
	}
	...
	protected static function clientChange($event,&$htmlOptions)
	{
		...
		if(isset($htmlOptions['submit']))
		{
			...
			if($htmlOptions['submit']!=='')
				...
				$url='';
			$handler.="jQuery.mod.submitForm(this,'$url',$params);{$return};";
		}
		...
		if($live)
			$cs->registerScript('Mod.CHtml.#' . $id,"jQuery('body').on('$event','#$id',function(){{$handler}});");
		else
			$cs->registerScript('Mod.CHtml.#' . $id,"jQuery('#$id').on('$event', function(){{$handler}});");
