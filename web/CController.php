...
class CController extends CBaseController
{
	 ...
	 */
	const STATE_INPUT_NAME='MOD_PAGE_STATE';
	...
	{
		throw new CHttpException(400,Mod::t('mod','Your request is invalid.'));
	}
	...
	{
		Mod::app()->getClientScript()->render($output);
	...
	public function createAction($actionID)
	{
		...
		else
		{
			...
			if($action!==null && !method_exists($action,'run'))
				throw new CException(Mod::t('mod', 'Action class {class} must implement the "run" method.', array('{class}'=>get_class($action))));
			return $action;
		}
	}
	...
	protected function createActionFromMap($actionMap,$actionID,$requestActionID,$config=array())
	{
		if(($pos=strpos($actionID,'.'))===false && isset($actionMap[$actionID]))
		{
			...
			return Mod::createComponent(empty($config)?$baseConfig:array_merge($baseConfig,$config),$this,$requestActionID);
		}
		elseif($pos===false)
			return null;
		...
		$class=Mod::import($providerType,true);
	...
	{
		throw new CHttpException(404,Mod::t('mod','The system is unable to find the requested action "{action}".',
	 ...
	 * The view file is determined by calling {@link ModBase::getPathOfAlias()}. Note that aliased views
	...
	{
		if(($theme=Mod::app()->getTheme())!==null && ($viewFile=$theme->getViewFile($this,$viewName))!==false)
			return $viewFile;
		$moduleViewPath=$basePath=Mod::app()->getViewPath();
		if(($module=$this->getModule())!==null)
	...
	public function getLayoutFile($layoutName)
	{
		if($layoutName===false)
			return false;
		if(($theme=Mod::app()->getTheme())!==null && ($layoutFile=$theme->getLayoutFile($this,$layoutName))!==false)
			return $layoutFile;
		...
		return $this->resolveViewFile($layoutName,$module->getLayoutPath(),Mod::app()->getViewPath(),$module->getViewPath());
	}
	...
	public function resolveViewFile($viewName,$viewPath,$basePath,$moduleViewPath=null)
	{
		...
		if(($renderer=Mod::app()->getViewRenderer())!==null)
		...
		elseif(strpos($viewName,'.'))
			$viewFile=Mod::getPathOfAlias($viewName);
		else
			$viewFile=$viewPath.DIRECTORY_SEPARATOR.$viewName;

		if(is_file($viewFile.$extension))
			return Mod::app()->findLocalizedFile($viewFile.$extension);
		elseif($extension!=='.php' && is_file($viewFile.'.php'))
			return Mod::app()->findLocalizedFile($viewFile.'.php');
		else
			return false;
	}
	...
	public function renderPartial($view,$data=null,$return=false,$processOutput=false)
	{
		if(($viewFile=$this->getViewFile($view))!==false)
		{
			...
			throw new CException(Mod::t('mod','{controller} cannot find the requested view "{view}".',
	...
	public function createUrl($route,$params=array(),$ampersand='&')
	{
		...
		if($route[0]!=='/' && ($module=$this->getModule())!==null)
			$route=$module->getId().'/'.$route;
		return Mod::app()->createUrl(trim($route,'/'),$params,$ampersand);
	}
	...
	public function createAbsoluteUrl($route,$params=array(),$schema='',$ampersand='&')
	{
		...
		if(strpos($url,'http')===0)
			...
			return Mod::app()->getRequest()->getHostInfo($schema).$url;
	}
	...
	public function getPageTitle()
	{
		...
		else
		{
			...
			if($this->getAction()!==null && strcasecmp($this->getAction()->getId(),$this->defaultAction))
				return $this->_pageTitle=Mod::app()->name.' - '.ucfirst($this->getAction()->getId()).' '.$name;
			else
				return $this->_pageTitle=Mod::app()->name.' - '.$name;
		}
	}
	...
	public function redirect($url,$terminate=true,$statusCode=302)
	{
		...
		Mod::app()->getRequest()->redirect($url,$terminate,$statusCode);
	}
	...
	{
		$this->redirect(Mod::app()->getRequest()->getUrl().$anchor,$terminate);
	}
	...
	{
		if(Mod::app()->getRequest()->getIsPostRequest())
			$filterChain->run();
		else
			throw new CHttpException(400,Mod::t('mod','Your request is invalid.'));
	}
	...
	{
		if(Mod::app()->getRequest()->getIsAjaxRequest())
			$filterChain->run();
		else
			throw new CHttpException(400,Mod::t('mod','Your request is invalid.'));
	}
	...
	protected function loadPageStates()
	{
		if(!empty($_POST[self::STATE_INPUT_NAME]))
		{
			if(($data=base64_decode($_POST[self::STATE_INPUT_NAME]))!==false)
			{
				...
				if(($data=Mod::app()->getSecurityManager()->validateData($data))!==false)
	...
	{
		$data=Mod::app()->getSecurityManager()->hashData(serialize($states));
		if(extension_loaded('zlib'))
