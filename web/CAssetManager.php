...
class CAssetManager extends CApplicationComponent
{
	...
	public function setBasePath($value)
	{
		if(($basePath=realpath($value))!==false && is_dir($basePath) && is_writable($basePath))
			...
			throw new CException(Mod::t('mod','CAssetManager.basePath "{path}" is invalid. Please make sure the directory exists and is writable by the Web server process.',
				array('{path}'=>$value)));
	}
	 ...
	 * discussion: http://code.google.com/p/mod/issues/detail?id=2579
	 *
	...
	public function publish($path,$hashByName=false,$level=-1,$forceCopy=null)
	{
		...
		if($forceCopy && $this->linkAssets)
			throw new CException(Mod::t('mod','The "forceCopy" and "linkAssets" cannot be both true.'));
		...
		throw new CException(Mod::t('mod','The asset "{asset}" to be published does not exist.',
			array('{asset}'=>$path)));
	}
	...
	{
		return sprintf('%x',crc32($path.Mod::getVersion()));
	}
