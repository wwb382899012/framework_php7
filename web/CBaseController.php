...
abstract class CBaseController extends CComponent
{
	...
	public function renderFile($viewFile,$data=null,$return=false)
	{
		...
		if(($renderer=Mod::app()->getViewRenderer())!==null && $renderer->fileExtension==='.'.CFileHelper::getExtension($viewFile))
		...
		else
		{
			...
			throw new CException(Mod::t('mod','{controller} contains improperly nested widget tags in its view "{view}". A {widget} widget does not have an endWidget() call.',
	...
	{
		$widget=Mod::app()->getWidgetFactory()->createWidget($this,$className,$properties);
		$widget->init();
		return $widget;
	}
	...
	public function endWidget($id='')
	{
		if(($widget=array_pop($this->_widgetStack))!==null)
		{
			...
			throw new CException(Mod::t('mod','{controller} has an extra endWidget({id}) call in its view.',
