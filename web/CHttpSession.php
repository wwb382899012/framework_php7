...
class CHttpSession extends CApplicationComponent implements IteratorAggregate,ArrayAccess,Countable
{
	...
	public function open()
	{
		...
		@session_start();
		if(MOD_DEBUG && session_id()=='')
		{
			$message=Mod::t('mod','Failed to start session.');
			...
			Mod::log($message, CLogger::LEVEL_WARNING, 'system.web.CHttpSession');
		}
	}
	...
	public function setSavePath($value)
	{
		if(is_dir($value))
			...
			throw new CException(Mod::t('mod','CHttpSession.savePath "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function setCookieMode($value)
	{
		...
		elseif($value==='only')
		{
			...
			throw new CException(Mod::t('mod','CHttpSession.cookieMode can only be "none", "allow" or "only".'));
	}
	...
	public function setGCProbability($value)
	{
		if($value>=0 && $value<=100)
		{
			...
			throw new CException(Mod::t('mod','CHttpSession.gcProbability "{value}" is invalid. It must be a float between 0 and 100.',
				array('{value}'=>$value)));
	}
