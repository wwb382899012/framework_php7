 ...
 *
 * CDbHttpSession stores session data in a DB table named 'ModSession'. The table name
 ...
 *
 * By default, it will use an SQLite3 database named 'session-ModVersion.db' under the application runtime directory.
...
class CDbHttpSession extends CHttpSession
{
	 ...
	 * is <code>protected/runtime/session-ModVersion.db</code>.
	 */
	public $connectionID;
	/**
	 ...
	 */
	public $sessionTableName='ModSession';
	/**
	...
	protected function getDbConnection()
	{
		...
		{
			if(($this->_db=Mod::app()->getComponent($id)) instanceof CDbConnection)
				return $this->_db;
			else
				throw new CException(Mod::t('mod','CDbHttpSession.connectionID "{id}" is invalid. Please make sure it refers to the ID of a CDbConnection application component.',
					array('{id}'=>$id)));
		}
		else
		{
			$dbFile=Mod::app()->getRuntimePath().DIRECTORY_SEPARATOR.'session-'.Mod::getVersion().'.db';
