...
class CWebApplication extends CApplication
{
	 ...
	 * {@link ModBase::getPathOfAlias class path alias} of the controller.
	 * If the latter, the array must contain a 'class' element which specifies
	 * the controller's class name or {@link ModBase::getPathOfAlias class path alias}.
	...
	public function createController($route,$owner=null)
	{
		...
		while(($pos=strpos($route,'/'))!==false)
		{
			...
			if(!isset($basePath))  // first segment
			{
				...
				{
					return array(
						Mod::createComponent($owner->controllerMap[$id],$id,$owner===$this?null:$owner),
	...
	public function setControllerPath($value)
	{
		if(($this->_controllerPath=realpath($value))===false || !is_dir($this->_controllerPath))
			throw new CException(Mod::t('mod','The controller path "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function setViewPath($path)
	{
		if(($this->_viewPath=realpath($path))===false || !is_dir($this->_viewPath))
			throw new CException(Mod::t('mod','The view path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
	...
	public function setSystemViewPath($path)
	{
		if(($this->_systemViewPath=realpath($path))===false || !is_dir($this->_systemViewPath))
			throw new CException(Mod::t('mod','The system view path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
	...
	public function setLayoutPath($path)
	{
		if(($this->_layoutPath=realpath($path))===false || !is_dir($this->_layoutPath))
			throw new CException(Mod::t('mod','The layout path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
