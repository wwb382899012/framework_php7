...
/**
 * CUrlManager manages the URLs of Mod Web applications.
 *
...
{
	const CACHE_KEY='Mod.CUrlManager.rules';
	...
	protected function processRules()
	{
		if(empty($this->rules) || $this->getUrlFormat()===self::GET_FORMAT)
			return;
		if($this->cacheID!==false && ($cache=Mod::app()->getComponent($this->cacheID))!==null)
		{
	...
	protected function createUrlRule($route,$pattern)
	{
		...
		{
			$urlRuleClass=Mod::import($this->urlRuleClass,true);
	...
	public function createUrl($route,$params=array(),$ampersand='&')
	{
		...
		{
			if(is_array($rule))
				$this->_rules[$i]=$rule=Mod::createComponent($rule);
	...
	public function parseUrl($request)
	{
		if($this->getUrlFormat()===self::PATH_FORMAT)
		{
			...
			if($this->useStrictParsing)
				throw new CHttpException(404,Mod::t('mod','Unable to resolve the request "{route}".',
	...
	public function getBaseUrl()
	{
		...
		{
			if($this->showScriptName)
				$this->_baseUrl=Mod::app()->getRequest()->getScriptUrl();
			else
				$this->_baseUrl=Mod::app()->getRequest()->getBaseUrl();
			return $this->_baseUrl;
		}
	}
	...
	public function setUrlFormat($value)
	{
		if($value===self::PATH_FORMAT || $value===self::GET_FORMAT)
			...
			throw new CException(Mod::t('mod','CUrlManager.UrlFormat must be either "path" or "get".'));
	}
}
...
class CUrlRule extends CBaseUrlRule
{
	...
	public function __construct($route,$pattern)
	{
		...
		if(MOD_DEBUG && @preg_match($this->pattern,'test')===false)
			throw new CException(Mod::t('mod','The URL pattern "{pattern}" for route "{route}" is not a valid regular expression.',
	...
	public function createUrl($manager,$route,$params,$ampersand)
	{
		...
		if($this->hasHostInfo)
		{
			$hostInfo=Mod::app()->getRequest()->getHostInfo();
