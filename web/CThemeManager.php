...
class CThemeManager extends CApplicationComponent
{
	...
	public function getTheme($name)
	{
		...
		if(is_dir($themePath))
		{
			$class=Mod::import($this->themeClass, true);
	...
	public function getBasePath()
	{
		if($this->_basePath===null)
			$this->setBasePath(dirname(Mod::app()->getRequest()->getScriptFile()).DIRECTORY_SEPARATOR.self::DEFAULT_BASEPATH);
	...
	public function setBasePath($value)
	{
		...
		if($this->_basePath===false || !is_dir($this->_basePath))
			throw new CException(Mod::t('mod','Theme directory "{directory}" does not exist.',array('{directory}'=>$value)));
	}
	...
	public function getBaseUrl()
	{
		if($this->_baseUrl===null)
			$this->_baseUrl=Mod::app()->getBaseUrl().'/'.self::DEFAULT_BASEPATH;
