...
class CHttpCacheFilter extends CFilter
{
	...
	public function preFilter($filterChain)
	{
		...
		if(!in_array(Mod::app()->getRequest()->getRequestType(), array('GET', 'HEAD')))
			return true;
	...
	protected function getLastModifiedValue()
	{
		if($this->lastModifiedExpression)
		{
			...
			elseif(($lastModified=strtotime($value))===false)
				throw new CException(Mod::t('mod','Invalid expression for CHttpCacheFilter.lastModifiedExpression: The evaluation result "{value}" could not be understood by strtotime()',
					array('{value}'=>$value)));
			return $lastModified;
		}
		...
		if($this->lastModified)
		{
			...
			elseif(($lastModified=strtotime($this->lastModified))===false)
				throw new CException(Mod::t('mod','CHttpCacheFilter.lastModified contained a value that could not be understood by strtotime()'));
			return $lastModified;
		}
		return false;
	}
	...
	{
		if(Mod::app()->session->isStarted)
		{
			Mod::app()->session->setCacheLimiter('public');
			header('Pragma:',true);
		}
