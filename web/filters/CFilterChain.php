...
class CFilterChain extends CList
{
	...
	public static function create($controller,$action,$filters)
	{
		...
		foreach($filters as $filter)
		{
			...
			{
				if(!isset($filter[0]))
					throw new CException(Mod::t('mod','The first element in a filter configuration must be the filter class.'));
				...
				$filter=Mod::createComponent($filter);
			}
	...
	public function insertAt($index,$item)
	{
		if($item instanceof IFilter)
			...
			throw new CException(Mod::t('mod','CFilterChain can only take objects implementing the IFilter interface.'));
	}
	...
	public function run()
	{
		if($this->offsetExists($this->filterIndex))
		{
			...
			Mod::trace('Running filter '.($filter instanceof CInlineFilter ? get_class($this->controller).'.filter'.$filter->name.'()':get_class($filter).'.filter()'),'system.web.filters.CFilterChain');
			$filter->filter($this);
		}
		else
