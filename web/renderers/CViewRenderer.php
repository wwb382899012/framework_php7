...
abstract class CViewRenderer extends CApplicationComponent implements IViewRenderer
{
	...
	public function renderFile($context,$sourceFile,$data,$return)
	{
		if(!is_file($sourceFile) || ($file=realpath($sourceFile))===false)
			throw new CException(Mod::t('mod','View file "{file}" does not exist.',array('{file}'=>$sourceFile)));
	...
	protected function getViewFile($file)
	{
		...
		{
			$crc=sprintf('%x', crc32(get_class($this).Mod::getVersion().dirname($file)));
			$viewFile=Mod::app()->getRuntimePath().DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR.$crc.DIRECTORY_SEPARATOR.basename($file);
			if(!is_file($viewFile))
