...
class CCaptcha extends CWidget
{
	...
	public function run()
	{
		if(self::checkRequirements('imagick') || self::checkRequirements('gd'))
		{
			...
			throw new CException(Mod::t('mod','GD with FreeType or ImageMagick PHP extensions are required.'));
	}
	...
	public function registerClientScript()
	{
		...
		if($this->showRefreshButton)
		{
			...
			$cs->registerScript('Mod.CCaptcha#'.$id,'// dummy');
			$label=$this->buttonLabel===null?Mod::t('mod','Get a new code'):$this->buttonLabel;
...
";
		$cs->registerScript('Mod.CCaptcha#'.$id,$js);
	}
