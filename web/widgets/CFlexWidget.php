...
class CFlexWidget extends CWidget
{
	...
	{
		if(empty($this->name))
			throw new CException(Mod::t('mod','CFlexWidget.name cannot be empty.'));
		if(empty($this->baseUrl))
			throw new CException(Mod::t('mod','CFlexWidget.baseUrl cannot be empty.'));
		if($this->altHtmlContent===null)
			$this->altHtmlContent=Mod::t('mod','This content requires the <a href="http://www.adobe.com/go/getflash/">Adobe Flash Player</a>.');
