...
class CClientScript extends CApplicationComponent
{
	...
	public function getCoreScriptUrl()
	{
		if($this->_baseUrl!==null)
			...
			return $this->_baseUrl=Mod::app()->getAssetManager()->publish(MOD_PATH.'/web/js/source');
	}
	...
	public function getPackageBaseUrl($name)
	{
		...
		if(isset($package['baseUrl']))
		{
			...
			if($baseUrl==='' || $baseUrl[0]!=='/' && strpos($baseUrl,'://')===false)
				$baseUrl=Mod::app()->getRequest()->getBaseUrl().'/'.$baseUrl;
			$baseUrl=rtrim($baseUrl,'/');
		}
		elseif(isset($package['basePath']))
			$baseUrl=Mod::app()->getAssetManager()->publish(Mod::getPathOfAlias($package['basePath']));
		else
	...
	public function hasPackage($name)
    {
        ...
        else
        {
            if($this->corePackages===null)
                $this->corePackages=require(MOD_PATH.'/web/js/packages.php');
	...
	public function registerCoreScript($name)
	{
		...
		elseif(MOD_DEBUG)
			...
			Mod::log('There is no CClientScript package: '.$name,CLogger::LEVEL_WARNING,'system.web.CClientScript');
	...
	public function registerScript($id,$script,$position=null,array $htmlOptions=array())
	{
		...
		else
		{
			if($position==self::POS_LOAD || $position==self::POS_READY)
				throw new CException(Mod::t('mod','Script HTML options are not allowed for "CClientScript::POS_LOAD" and "CClientScript::POS_READY".'));
	 ...
	 * This is a shortcut to Mod::app()->controller->recordCachingAction.
	...
	{
		if(($controller=Mod::app()->getController())!==null)
