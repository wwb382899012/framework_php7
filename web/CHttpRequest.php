 ...
 * @property string $hostInfo Schema and hostname part (with port number if needed) of the request URL (e.g. http://www.modframework.com).
...
class CHttpRequest extends CApplicationComponent
{
	 ...
	 * By setting this property to true, forms submitted to an Mod Web application must be originated
	...
	/**
	 * @var string the name of the token used to prevent CSRF. Defaults to 'MOD_CSRF_TOKEN'.
	 * This property is effectively only when {@link enableCsrfValidation} is true.
	 */
	public $csrfTokenName='MOD_CSRF_TOKEN';
	/**
	 ...
	 * @return string schema and hostname part (with port number if needed) of the request URL (e.g. http://www.modframework.com)
	 * @see setHostInfo
	 */
	...
	public function getScriptUrl()
	{
		if($this->_scriptUrl===null)
		{
			...
			elseif(isset($_SERVER['DOCUMENT_ROOT']) && strpos($_SERVER['SCRIPT_FILENAME'],$_SERVER['DOCUMENT_ROOT'])===0)
				...
				throw new CException(Mod::t('mod','CHttpRequest is unable to determine the entry script URL.'));
		}
	...
	public function getPathInfo()
	{
		if($this->_pathInfo===null)
		{
			...
			elseif(strpos($_SERVER['PHP_SELF'],$scriptUrl)===0)
				...
				throw new CException(Mod::t('mod','CHttpRequest is unable to determine the path info of the request.'));
	...
	public function getRequestUri()
	{
		if($this->_requestUri===null)
		{
			...
			elseif(isset($_SERVER['ORIG_PATH_INFO']))  // IIS 5.0 CGI
			{
				...
				throw new CException(Mod::t('mod','CHttpRequest is unable to determine the request URI.'));
		}
	 ...
	 * <pre>
	 * <?php
	 *    Mod::app()->request->xSendFile('/home/user/Pictures/picture1.jpg',array(
	 *        'saveName'=>'image1.jpg',
	 *        'mimeType'=>'image/jpeg',
	 *        'terminate'=>false,
	 *    ));
	...
	public function validateCsrfToken($event)
	{
			...
			$this->getIsDeleteRequest())
		{
			...
			if (!empty($maskedUserToken) && $cookies->contains($this->csrfTokenName))
			{
				...
				$valid = false;
			if (!$valid)
				throw new CHttpException(400,Mod::t('mod','The CSRF token could not be verified.'));
		}
	}
...
class CCookieCollection extends CMap
{
	...
	public function add($name,$cookie)
	{
		if($cookie instanceof CHttpCookie)
		{
			...
			throw new CException(Mod::t('mod','CHttpCookieCollection can only hold CHttpCookie objects.'));
	}
	 ...
	 * Mod::app()->request->cookies['foo']=new CHttpCookie('cookie','value',$options);
	 * Mod::app()->request->cookies->remove('cookie',$options);
	 * </pre>
	 *
	 * @param mixed $name Cookie name.
	...
	protected function addCookie($cookie)
	{
		$value=$cookie->value;
		if($this->_request->enableCookieValidation)
			$value=Mod::app()->getSecurityManager()->hashData(serialize($value));
