...
class CDbAuthManager extends CAuthManager
{
	...
	protected function checkAccessRecursive($itemName,$userId,$params,$assignments)
	{
		if(($item=$this->getAuthItem($itemName))===null)
			return false;
		Mod::trace('Checking permission "'.$item->getName().'"','system.web.auth.CDbAuthManager');
	...
	public function addItemChild($itemName,$childName)
	{
		if($itemName===$childName)
			throw new CException(Mod::t('mod','Cannot add "{name}" as a child of itself.',
					array('{name}'=>$itemName)));
		...
		if(count($rows)==2)
		{
			...
			if($this->detectLoop($itemName,$childName))
				throw new CException(Mod::t('mod','Cannot add "{child}" as a child of "{name}". A loop has been detected.',
	...
	public function assign($itemName,$userId,$bizRule=null,$data=null)
	{
		if($this->usingSqlite() && $this->getAuthItem($itemName)===null)
			throw new CException(Mod::t('mod','The item "{name}" does not exist.',array('{name}'=>$itemName)));
	...
	protected function getDbConnection()
	{
		if($this->db!==null)
			return $this->db;
		elseif(($this->db=Mod::app()->getComponent($this->connectionID)) instanceof CDbConnection)
			return $this->db;
		else
			throw new CException(Mod::t('mod','CDbAuthManager.connectionID "{id}" is invalid. Please make sure it refers to the ID of a CDbConnection application component.',
