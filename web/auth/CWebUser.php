...
{
	const FLASH_KEY_PREFIX='Mod.CWebUser.flash.';
	const FLASH_COUNTERS='Mod.CWebUser.flashcounters';
		...
		if($this->beforeLogin($id,$states,false))
		{
			...
			if($duration>0)
			{
				if($this->allowAutoLogin)
					...
					throw new CException(Mod::t('mod','{class}.allowAutoLogin must be set true in order to use cookie-based authentication.',
	...
	public function logout($destroySession=true)
	{
		if($this->beforeLogout())
		{
			if($this->allowAutoLogin)
			{
				Mod::app()->getRequest()->getCookies()->remove($this->getStateKeyPrefix());
				if($this->identityCookie!==null)
				{
					$cookie=$this->createIdentityCookie($this->getStateKeyPrefix());
					$cookie->value=null;
					$cookie->expire=0;
					Mod::app()->getRequest()->getCookies()->add($cookie->name,$cookie);
				}
			}
			if($destroySession)
	...
	public function getReturnUrl($defaultUrl=null)
	{
		if($defaultUrl===null)
		{
			$defaultReturnUrl=Mod::app()->getUrlManager()->showScriptName ? Mod::app()->getRequest()->getScriptUrl() : Mod::app()->getRequest()->getBaseUrl().'/';
		}
		else
		{
	...
	protected function renewCookie()
	{
		...
		if($cookie && !empty($cookie->value) && ($data=Mod::app()->getSecurityManager()->validateData($cookie->value))!==false)
		{
			$data=@unserialize($data);
			if(is_array($data) && isset($data[0],$data[1],$data[2],$data[3]))
			{
	...
	public function getStateKeyPrefix()
	{
		if($this->_keyPrefix!==null)
			...
			return $this->_keyPrefix=md5('Mod.'.get_class($this).'.'.Mod::app()->getId());
	}
	...
	{
		Mod::app()->getSession()->regenerateID(true);
		$this->setId($id);
		$this->setName($name);
	...
	public function checkAccess($operation,$params=array(),$allowCaching=true)
	{
		...
		$access=Mod::app()->getAuthManager()->checkAccess($operation,$this->getId(),$params);
