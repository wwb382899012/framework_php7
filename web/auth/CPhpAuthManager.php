...
class CPhpAuthManager extends CAuthManager
{
	...
	public function init()
	{
		...
		if($this->authFile===null)
			$this->authFile=Mod::getPathOfAlias('application.data.auth').'.php';
		$this->load();
	}
	...
	public function checkAccess($itemName,$userId,$params=array())
	{
		...
		Mod::trace('Checking permission "'.$item->getName().'"','system.web.auth.CPhpAuthManager');
	...
	public function addItemChild($itemName,$childName)
	{
		if(!isset($this->_items[$childName],$this->_items[$itemName]))
			throw new CException(Mod::t('mod','Either "{parent}" or "{child}" does not exist.',array('{child}'=>$childName,'{parent}'=>$itemName)));
		$child=$this->_items[$childName];
		$item=$this->_items[$itemName];
		$this->checkItemChildType($item->getType(),$child->getType());
		if($this->detectLoop($itemName,$childName))
			throw new CException(Mod::t('mod','Cannot add "{child}" as a child of "{parent}". A loop has been detected.',
				array('{child}'=>$childName,'{parent}'=>$itemName)));
		if(isset($this->_children[$itemName][$childName]))
			throw new CException(Mod::t('mod','The item "{parent}" already has a child "{child}".',
				array('{child}'=>$childName,'{parent}'=>$itemName)));
	...
	{
		if(!isset($this->_items[$itemName]))
			throw new CException(Mod::t('mod','Unknown authorization item "{name}".',array('{name}'=>$itemName)));
		elseif(isset($this->_assignments[$userId][$itemName]))
			throw new CException(Mod::t('mod','Authorization item "{item}" has already been assigned to user "{user}".',
	...
	public function createAuthItem($name,$type,$description='',$bizRule=null,$data=null)
	{
		if(isset($this->_items[$name]))
			throw new CException(Mod::t('mod','Unable to add an item whose name is the same as an existing item.'));
	...
	public function saveAuthItem($item,$oldName=null)
	{
		if($oldName!==null && ($newName=$item->getName())!==$oldName) // name changed
		{
			if(isset($this->_items[$newName]))
				throw new CException(Mod::t('mod','Unable to change the item name. The name "{name}" is already used by another item.',array('{name}'=>$newName)));
