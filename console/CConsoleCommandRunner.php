...
class CConsoleCommandRunner extends CComponent
{
	 ...
	 * {@link ModBase::getPathOfAlias class path alias} of the command.
	 * If the latter, the array must contain a 'class' element which specifies
	 * the command's class name or {@link ModBase::getPathOfAlias class path alias}.
		...
		if($command!==null)
		{
			if(is_string($command)) // class file path or alias
			{
				if(strpos($command,'/')!==false || strpos($command,'\\')!==false)
				{
					...
					$className=Mod::import($command);
				return new $className($name,$this);
			}
			else // an array configuration
				return Mod::createComponent($command,$name,$this);
		}
		elseif($name==='help')
