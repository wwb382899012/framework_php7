...
class CLogFilter extends CComponent implements ILogFilter
{
	...
	public function filter(&$logs)
	{
		if (!empty($logs))
		{
			if(($message=$this->getContext())!=='')
				array_unshift($logs,array($message,CLogger::LEVEL_INFO,'application',MOD_BEGIN_TIME));
			$this->format($logs);
		}
		return $logs;
	}
	...
	protected function format(&$logs)
	{
		...
		if($this->prefixSession && ($id=session_id())!=='')
			$prefix.="[$id]";
		if($this->prefixUser && ($user=Mod::app()->getComponent('user',false))!==null)
	...
	{
		$context=array();
		if($this->logUser && ($user=Mod::app()->getComponent('user',false))!==null)
