...
class CFileLogRoute extends CLogRoute
{
	...
	public function init()
	{
		...
		if($this->getLogPath()===null)
			$this->setLogPath(Mod::app()->getRuntimePath());
	}
	...
	public function setLogPath($value)
	{
		...
		if($this->_logPath===false || !is_dir($this->_logPath) || !is_writable($this->_logPath))
			throw new CException(Mod::t('mod','CFileLogRoute.logPath "{path}" does not point to a valid directory. Make sure the directory exists and is writable by the Web server process.',
				array('{path}'=>$value)));
	}
