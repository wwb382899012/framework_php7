 ...
 * application component. If they are not set, a SQLite3 database named 'log-ModVersion.db' will be created
...
class CDbLogRoute extends CLogRoute
{
	 ...
	 * <code>protected/runtime/log-ModVersion.db</code>.
	 */
	public $connectionID;
	/**
	 * @var string the name of the DB table that stores log content. Defaults to 'ModLog'.
	...
	protected function getDbConnection()
	{
		...
		elseif(($id=$this->connectionID)!==null)
		{
			if(($this->_db=Mod::app()->getComponent($id)) instanceof CDbConnection)
				...
				throw new CException(Mod::t('mod','CDbLogRoute.connectionID "{id}" does not point to a valid CDbConnection application component.',
					array('{id}'=>$id)));
		}
		else
		{
			$dbFile=Mod::app()->getRuntimePath().DIRECTORY_SEPARATOR.'log-'.Mod::getVersion().'.db';
