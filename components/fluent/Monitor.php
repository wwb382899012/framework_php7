...
class Monitor extends CApplicationComponent{
    ...
    {
        if($module_id===null) {
            if (self::$log_tag=="" && isset(Mod::app()->id))
                self::$log_tag = Mod::app()->id;
        }
        else{
     ...
     *      postfix     tag的后缀
     */
    static public function log_custom($postfix)
    {
        $arg_arr=func_get_args();
        $info_arr=array();
        $last_key=null;
